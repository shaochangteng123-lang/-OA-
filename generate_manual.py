...
def create_manual():
    ...
    add_bullet(doc, '基础报销：日常费用（办公、差旅、服务等），按现有基础报销规则办理')
    add_bullet(doc, '大额报销：不设金额限制，品类与基础报销相同')
    ...
    data2 = [
        ['报销类型', '适用规则', '审批人', '典型场景'],
        ['基础报销', '日常费用', '管理员', '办公耗材、快递、差旅、茶水等'],
        ['大额报销', '不设金额限制', '管理员', '设备采购、服务费等'],
        ...
        for j, text in enumerate(row_data):
            ...
            for para in cell.paragraphs:
                for run in para.runs:
                    set_font(run, size=10, bold=(i==0))
    doc.add_paragraph()
    add_tip(doc, '基础报销和大额报销不再按 1000 元强制分流，请按费用性质及内部管理要求选择；商务活动请选择商务报销。')

    add_h2(doc, '3.2  创建基础报销单')
    add_para(doc, '适用于日常费用报销。')
    doc.add_paragraph()
    ...
    add_para(doc, '大额报销不设金额限制，操作流程与基础报销完全相同。')
    ...
    add_tip(doc, '系统不再按 1000 元门槛拦截大额报销，请按实际费用正常填写并提交。')
