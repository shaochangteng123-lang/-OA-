...
router.get("/", requireAuth, async (req, res) => {
  try {
    ...
    const { q, district, ownerUserId, isCompleted } = req.query as Record<
      string,
      string
    >;
    ...
    if (!admin) {
      where.push(
        `(p.owner_user_id = ? OR EXISTS (SELECT 1 FROM worklog_entries e WHERE e.project_id = p.id AND e.user_id = ?))`,
      );
    ...
    if (isCompleted === "true") {
      where.push(`p.is_completed = TRUE`);
    }
    if (isCompleted === "false") {
      where.push(`p.is_completed = FALSE`);
    }
    ...
    res.json({
      success: true,
      data: rows.map((r) => ({ ...toApi(r), entryCount: r.entry_count })),
    });
  } catch (err) {
...
router.post("/", requireAuth, async (req, res) => {
  try {
    ...
    if (
      ...
      !clientContactPhone?.trim() ||
      !district ||
      !projectType ||
      !ownerUserId ||
      !agencyBureau?.trim() ||
      !agencyDepartment?.trim() ||
      !agencyContactName?.trim() ||
      !agencyContactPhone?.trim()
    ) {
      return res
        .status(400)
        .json({
          success: false,
          message:
            "项目名称、甲方单位、甲方负责人、行政区、项目类型、负责人、办理机构信息均为必填",
        });
    }
    ...
    if (existing)
      return res.json({ success: true, data: toApi(existing), existed: true });

    const owner = await db.get<{ name: string; position: string | null }>(
      `SELECT name, position FROM users WHERE id = ?`,
      ownerUserId,
    );
    if (!owner)
      return res
        .status(400)
        .json({ success: false, message: "负责人用户不存在" });
    ...
    await db.run(
      ...
      owner.position,
      sd,
      agencyBureau?.trim() || null,
      agencyDepartment?.trim() || null,
      agencyContactName?.trim() || null,
      agencyContactPhone?.trim() || null,
      userId,
      now,
      now,
    );
    ...
    const row = await db.get<ProjectRow>(
      `SELECT * FROM worklog_projects WHERE id = ?`,
      id,
    );
...
router.put("/:id", requireAuth, async (req, res) => {
  try {
    ...
    if (row.owner_user_id !== userId && !(await isAdminLike(userId))) {
      return res
        .status(403)
        .json({ success: false, message: "仅负责人或管理员可修改项目" });
    }
    ...
    if (clientContactName !== undefined) {
      sets.push("client_contact_name = ?");
      params.push(clientContactName.trim());
    }
    if (clientContactPhone !== undefined) {
      sets.push("client_contact_phone = ?");
      params.push(clientContactPhone.trim());
    }
    ...
    if (agencyBureau !== undefined) {
      sets.push("agency_bureau = ?");
      params.push(agencyBureau.trim() || null);
    }
    if (agencyDepartment !== undefined) {
      sets.push("agency_department = ?");
      params.push(agencyDepartment.trim() || null);
    }
    if (agencyContactName !== undefined) {
      sets.push("agency_contact_name = ?");
      params.push(agencyContactName.trim() || null);
    }
    if (agencyContactPhone !== undefined) {
      sets.push("agency_contact_phone = ?");
      params.push(agencyContactPhone.trim() || null);
    }
    ...
    if (sets.length === 0)
      return res
        .status(400)
        .json({ success: false, message: "没有可更新字段" });
    ...
    await db.run(
      `UPDATE worklog_projects SET ${sets.join(", ")} WHERE id = ?`,
      ...params,
    );
...
router.post("/:id/complete", requireAuth, async (req, res) => {
  try {
    ...
    if (row.owner_user_id !== userId && !(await isAdminLike(userId))) {
      return res
        .status(403)
        .json({ success: false, message: "仅负责人或管理员可办结项目" });
    }
    const now = new Date().toISOString();
    await db.run(
...
router.get("/:id/gantt", requireAuth, async (req, res) => {
  try {
    ...
    const matterDict = await db.all<{
      name: string;
      standard_days: number | null;
    }>(`SELECT name, standard_days FROM worklog_matters`);
    const standardMap = new Map(
      matterDict.map((m) => [m.name, m.standard_days]),
    );

    const data = rows.map((r) => {
      const isAllFinalized =
        r.total_count > 0 && r.total_count === r.finalized_count;
      // 已办结则截止到最后日志日期，否则到今天
      ...
      const isOverdue =
        standardDays !== null && !isAllFinalized && daysElapsed > standardDays;
      return {
        matter: r.matter,
    ...
    res.json({
      success: true,
      data: { project: toApi(project), matters: data },
    });
  } catch (err) {
...
interface FormalContractSummary {
  id: string;
  status: string;
  title: string | null;
}

async function getFormalContractForProject(
  projectId: string,
): Promise<FormalContractSummary | undefined> {
  return db.get<FormalContractSummary>(
    `SELECT COALESCE(root_contract_id, id) AS id, status, title
     FROM contracts
     WHERE project_id = ? AND is_deleted = FALSE
     ORDER BY
       CASE relation_type WHEN 'main' THEN 0 ELSE 1 END,
       created_at DESC
     LIMIT 1`,
    projectId,
  );
...
function formalContractManagedResponse(formalContract: FormalContractSummary) {
  return {
    success: false,
    code: "FORMAL_CONTRACT_MANAGED",
    message: "该项目已启用正式合同管理，请前往合同管理模块操作",
    data: { formalContractId: formalContract.id },
  };
}
...
router.get("/:id/contract", requireAuth, async (req, res) => {
  try {
    ...
    const formalContract = await getFormalContractForProject(req.params.id);
    ...
    res.json({
      ...
      data: {
        ...
        formalContractId: formalContract?.id || null,
        formalContractStatus: formalContract?.status || null,
        progress: data,
      },
    });
  } catch (err) {
...
router.post("/:id/contract", requireAuth, async (req, res) => {
  try {
    ...
    if (formalContract) {
      return res
        .status(409)
        .json(formalContractManagedResponse(formalContract));
    }

    const { status, amount, note } = req.body as {
      status?: string;
      amount?: number;
      note?: string;
    };
    if (!status?.trim()) {
      return res
        .status(400)
        .json({ success: false, message: "合同状态不能为空" });
    }

    const me = await db.get<{ name: string }>(
...
router.delete("/:id/contract/:progressId", requireAuth, async (req, res) => {
  try {
    ...
    if (!progress)
      return res
        .status(404)
        .json({ success: false, message: "进度记录不存在" });

    const admin = await isAdminLike(userId);
    if (!admin && progress.created_by !== userId) {
      return res
        .status(403)
        .json({ success: false, message: "只能删除自己的进度记录" });
    }

    // 删除关联附件文件
    const atts = await db.all<ContractAttachmentRow>(
      `SELECT * FROM worklog_contract_attachments WHERE progress_id = ?`,
      req.params.progressId,
    );
    await db.run(
      `DELETE FROM worklog_contract_progress WHERE id = ?`,
      req.params.progressId,
    );
    for (const a of atts) {
      try {
...
router.post(
  "/:id/contract/:progressId/attachments",
  requireAuth,
  (req, res, next) => {
    (async () => {
      const formalContract = await getFormalContractForProject(req.params.id);
...
router.delete(
  "/:id/contract/:progressId/attachments/:attId",
  requireAuth,
  async (req, res) => {
    try {
      const userId = req.session.userId!;
      ...
      if (!admin && att.uploaded_by !== userId) {
        return res
          .status(403)
          .json({ success: false, message: "只能删除自己上传的附件" });
      }

      await db.run(
        `DELETE FROM worklog_contract_attachments WHERE id = ?`,
        att.id,
      );
      try {
...
router.put("/:id/contract-amount", requireAuth, async (req, res) => {
  try {
    ...
    if (project.owner_user_id !== userId && !(await isAdminLike(userId))) {
      return res
        .status(403)
        .json({ success: false, message: "仅负责人或管理员可修改" });
    }
