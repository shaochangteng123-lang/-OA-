...
async function main() {
  ...
  try {
    await db.transaction(async (client) => {
      ...
      `)
      console.log(`✅ 删除 ${approvalRecordsResult.rowCount ?? 0} 条审批记录`)

      // 删除审批实例
      const approvalInstancesResult = await client.query(`
        DELETE FROM approval_instances WHERE target_type = 'reimbursement'
      `)
      console.log(`✅ 删除 ${approvalInstancesResult.rowCount ?? 0} 条审批实例`)

      // 删除发票明细
      const invoicesResult = await client.query(`DELETE FROM reimbursement_invoices`)
      console.log(`✅ 删除 ${invoicesResult.rowCount ?? 0} 条发票明细`)

      // 删除核减统计
      const deductionsResult = await client.query(`DELETE FROM reimbursement_deductions`)
      console.log(`✅ 删除 ${deductionsResult.rowCount ?? 0} 条核减统计`)

      // 删除报销单
      const reimbursementsResult = await client.query(`DELETE FROM reimbursements`)
      console.log(`✅ 删除 ${reimbursementsResult.rowCount ?? 0} 条报销单`)
    })
