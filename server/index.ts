...
import contractsRoutes, {
  resumePendingContractRecognitionJobs,
} from "./routes/contracts.js";
import contractSealApplicationsRoutes from "./routes/contractSealApplications.js";
import contractAuxiliaryPackagesRoutes, {
  resumePendingContractAuxiliaryRecognitionJobs,
} from "./routes/contractAuxiliaryPackages.js";
import contractDownloadRequestsRoutes from "./routes/contractDownloadRequests.js";
import invoiceApplicationsRoutes from "./routes/invoice-applications.js";
import monthlyFinancialReportsRoutes from "./routes/monthly-financial-reports.js";
import { shutdownOcrDaemon } from "./services/ocrDaemon.js";
import { shutdownTesseractOcrDaemon } from "./services/tesseractOcrDaemon.js";
...
const isLocalhost =
  process.env.FRONTEND_URL?.includes("localhost") || !process.env.FRONTEND_URL;
...
app.use("/api/contracts", contractSealApplicationsRoutes);
app.use("/api/contracts", contractAuxiliaryPackagesRoutes);
app.use("/api/contracts", contractsRoutes);
app.use("/api/contract-download-requests", contractDownloadRequestsRoutes);
app.use("/api/invoice-applications", invoiceApplicationsRoutes);
app.use("/api/monthly-financial-reports", monthlyFinancialReportsRoutes);
  ...
  const runReset = async () => {
    try {
      ...
      const types = await db
        .prepare(
          `SELECT code, default_days FROM leave_type_configs WHERE is_active = true`,
        )
        .all<{ code: string; default_days: number }>();
      ...
      for (const user of users) {
        for (const type of types) {
          ...
          const existing = await db
            ...
            .get(user.id, type.code, year);
          if (existing) {
            // 已存在记录：重置已用天数和待审批天数，重新计算总天数（确保真正清零）
            await db
              .prepare(
                `UPDATE leave_balances SET total_days = ?, used_days = 0, pending_days = 0, updated_at = ? WHERE user_id = ? AND leave_type_code = ? AND year = ?`,
              )
              .run(totalDays, nowStr, user.id, type.code, year);
            updated++;
          } else {
            const { nanoid } = await import("nanoid");
            await db
              .prepare(
                `INSERT INTO leave_balances (id, user_id, leave_type_code, year, total_days, used_days, pending_days, created_at, updated_at) VALUES (?, ?, ?, ?, ?, 0, 0, ?, ?)`,
              )
              .run(
                nanoid(),
                user.id,
      ...
      console.log(
        `✅ ${year} 年假期余额重置完成：新建 ${initialized} 条，更新清零 ${updated} 条`,
      );
    } catch (error) {
...
async function start() {
  ...
  await initDatabase();
  const legacyContractDraftCleanup = await cleanupLegacyDeletedContractDrafts();
  if (
    legacyContractDraftCleanup.deletedDraftCount > 0 ||
    legacyContractDraftCleanup.renumberedSupplementCount > 0 ||
    legacyContractDraftCleanup.blockedDraftCount > 0 ||
    legacyContractDraftCleanup.failedFilePaths.length > 0
  ) {
    console.log("合同历史软删除草稿清理完成:", {
      deletedDraftCount: legacyContractDraftCleanup.deletedDraftCount,
      renumberedSupplementCount:
        legacyContractDraftCleanup.renumberedSupplementCount,
      deletedFileCount: legacyContractDraftCleanup.deletedFileCount,
      failedFileCount: legacyContractDraftCleanup.failedFilePaths.length,
      blockedDraftCount: legacyContractDraftCleanup.blockedDraftCount,
      blockedContractIds: legacyContractDraftCleanup.blockedContractIds,
    });
  }
  await seedDatabase();
  const legacyApprovalMigration =
    await reassignLegacyPendingContractApprovalRounds();
  if (legacyApprovalMigration.reassignedCount > 0) {
    console.log(
      `合同历史项目负责人待审批轮次已统一迁移至总经理：${legacyApprovalMigration.reassignedCount} 条`,
    );
  }
  const financialSettlementBackfill =
    await backfillLegacyPostedContractFinancialSettlements();
  if (
    financialSettlementBackfill.postedRegistrationCount > 0 ||
    financialSettlementBackfill.skippedRegistrationIds.length > 0
  ) {
    console.log("合同历史部分结算即时入账补处理完成:", {
      postedRegistrationCount:
        financialSettlementBackfill.postedRegistrationCount,
      postedSettlementCount: financialSettlementBackfill.postedSettlementCount,
      skippedRegistrationIds:
        financialSettlementBackfill.skippedRegistrationIds,
    });
  }
  await resumePendingContractRecognitionJobs();
  ...
  const { setupHolidayAutoUpdater } =
    await import("./services/holidayUpdater.js");
  setupHolidayAutoUpdater();

  // 日志自动归档 + 周报自动生成
  const { setupDailyLogScheduler } =
    await import("./services/dailyLogScheduler.js");
    ...
    if (clients) {
      for (const client of clients) {
        try {
          ...
          // 关闭阶段尽力终止连接，单个连接失败不阻断进程退出。
        }
      }
    }
  } catch {
    // 兼容不同连接池实现，无法访问内部连接集合时继续标准关闭。
  }
  try {
    await Promise.race([pool.end(), new Promise((r) => setTimeout(r, 2000))]);
  } catch {
    // 关闭失败时仍按信号终止，避免开发热重载留下僵尸进程。
  }
  process.exit(0);
}
