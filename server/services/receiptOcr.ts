...
async function recognizeReceiptInternal(
  ...
  try {
    ...
    console.log(
      `✅ 识别完成！金额: ${result.amount}，耗时: ${Date.now() - startTime}ms`,
    );
    return result;
  } catch (error) {
...
export function parseRecognizedReceiptText(text: string): ReceiptOcrResult {
  ...
  const hasSuccessKeyword = successKeywords.some((kw) =>
    textNoSpace.includes(kw),
  );
  // 支付宝实物订单付款后、确认收货前显示“等待确认收货”，仍属于已发生支出。
  // 必须同时具备账单详情、支付时间、付款方式和负向金额，避免把未付款订单页放行。
  const isPaidAlipayPendingReceipt =
    textNoSpace.includes("账单详情") &&
    textNoSpace.includes("等待确认收货") &&
    textNoSpace.includes("支付时间") &&
    textNoSpace.includes("付款方式") &&
    extractExplicitAmounts(text).length > 0;

  if (!hasSuccessKeyword && !isPaidAlipayPendingReceipt) {
    console.log("❌ 未检测到可靠支付完成证据");
    throw new Error("此不是支付截图，请重新上传");
  }
  ...
  if (result.amount === 0) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      const m = lines[i].match(
        /^([¥￥-])?\s*((?:\d{1,3}(?:[,，]\d{3})+|\d{1,6})(?:\.\d{1,2})?)\s*$/,
      );
      if (!m) continue;

      const hasSign = !!m[1]; // 是否有负号或货币符号
      const normalizedAmountText = m[2].replace(/[,，]/g, "");
      ...
      if (!hasSign) {
        ...
        const decStr = normalizedAmountText.includes(".")
          ? normalizedAmountText.split(".")[1]
          : "";
        ...
        if (i <= 2 && !normalizedAmountText.includes(".") && amount <= 100) {
          console.log(
            `⏭️ 跳过疑似状态栏数字: ${normalizedAmountText} (行 ${i})`,
          );
          continue;
        }
      }
    ...
    if (candidates.length > 0) {
      ...
      if (signedCandidates.length > 0) {
        ...
        console.log(
          ...
          result.amount,
          "(行",
          signedCandidates[0].lineIdx,
          ")",
        );
      } else if (anchorLineIdx > 0) {
        // 策略2：没有带符号的，在关键词行之前的范围内找最靠近关键词的那个
        // （大金额行紧靠在"支付成功"/"当前状态"之前）
        const beforeAnchor = candidates.filter(
          (c) => c.lineIdx < anchorLineIdx,
        );
