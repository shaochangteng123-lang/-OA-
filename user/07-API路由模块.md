...
GET    /api/auth/signature         // 按 type 获取已保存签名状态
GET    /api/auth/signature/image   // 按 type 读取已保存签名图片
POST   /api/auth/signature         // 按 signatureType 保存或更换电子签名
DELETE /api/auth/signature         // 按 type 删除已保存电子签名
POST   /api/auth/logout            // 退出登录
```

- 电子签名类型支持 `personal`（本人签名）和 `general_manager`（总经理签名）；未传类型时默认本人签名
- 本人签名接口不接收用户编号，只依据当前登录会话操作本人数据；总经理签名只允许管理员或超级管理员维护，并绑定当前活动总经理账号及姓名
- 保存时继续执行签名图片去底、阴影过滤、笔画筛选和裁切；图片通过鉴权接口返回，不开放上传目录物理路径
- 转正签署调用已保存签名后，仍在当前申请版本中复制独立签名快照；个人设置后续更换或删除不会修改历史流程文件
...
POST   /api/users                              // 管理员更新用户；BOSS仅维护轻量账号字段
POST   /api/users/create                       // 管理员创建员工账号或轻量BOSS账号
...
- 精简目录只返回编号、姓名、头像、部门、职位和角色，供普通业务选择器使用；BOSS不进入该目录
...
- 四类文件均复用财务区审批中心付款回单的解析和真实性校验；PDF（便携式文档格式）按页转换图片，并兼容同页上下两笔回单。金额按整数分汇总，部分页面失败时只汇总校验成功金额
...
- 实发工资回单优先按完整收款账号匹配员工，账号缺失时按收款户名精确匹配；同名或无法唯一匹配时不自动关联。工资表每行返回 `salary_receipts`，点击员工实发金额后通过单笔预览接口查看对应页及上下区域
...
- 管理员和超级管理员额外返回 `probationArchivePending`，统计已经完成全部转正审批、但当前申请版本尚未上传 `official` 正式盖章档案的员工
...
GET  /api/probation/list                  // 管理员或总经理按职责读取转正管理列表
GET  /api/probation/:id                   // 管理员或总经理读取申请详情和当前版本签名记录
...
- 转正必须按员工、主管领导意见、人事部意见、总经理审批顺序处理；主管领导意见分配给活动总经理账号，人事部意见和总经理审批均由管理员或超级管理员处理
- 参与人必须先在个人设置上传签名；转正接口只接收 `signatureType`，由服务器读取、处理并复制已保存签名，不接收客户端任意签名图片。管理员签署时可选择本人签名或其代管的总经理签名
- 每条签署记录同时保存实际登录操作人和所选签名人；最终总经理审批使用代管的总经理签名时，实际操作人仍记录为管理员，避免代签审计信息丢失
- `my-status`、`signature-tasks` 和申请详情同时返回当前版本 `signatures` 与全部版本 `signatureHistory`；历史记录包含版本、环节、决定、签名人、实际操作人、意见和服务端签署时间
- 签名图片读取接口只允许申请人、对应主管、原签署人、管理员、超级管理员或总经理访问，不返回物理存储路径
...
- 管理员签署人事部意见并通过后，前端刷新同一申请并在当前窗口自动切换到总经理审批；最终签名区随即开放，无需退出后重新进入
- 管理员完成最终审批时写入转正完成状态、员工在职状态和终审时间，并基于当前版本生成唯一的待盖章 PDF（便携式文档格式）底稿
...
- 总经理使用 `reviewedByMeThisMonth=1` 查询本人本月实际签署过主管领导意见的申请，通过和驳回都计入，签署后无需等待管理员终审即可查看；该参数拒绝管理员或超级管理员调用
...
- 扫描标题漏识别时使用各固定类型的正文组合特征确认首页；低分辨率结果仅匹配到可疑“其他”标题时提高分辨率复核，复核后仍不匹配才归入“其他”
...
GET    /api/leave/pending                       // 指定总经理获取本人待审批请假
GET    /api/leave/reviewed                      // 总经理获取本人历史审批记录
POST   /api/leave/requests/:id/approve          // 指定总经理审批通过
POST   /api/leave/requests/:id/reject           // 指定总经理审批驳回
...
- 请假提交时只绑定申请人之外的活动状态总经理；没有可用总经理时拒绝提交并提示管理员配置总经理账号，不再回退到超级管理员
- 待办、审批记录、通过和驳回接口只允许总经理调用，并且只能处理 `approver_id`（审批人编号）等于当前账号的申请
- `GET /api/approval/pending-counts` 只为总经理统计分配给本人的 `leaveApprovalPending`（请假待审批数量）；管理员和超级管理员固定为 0
