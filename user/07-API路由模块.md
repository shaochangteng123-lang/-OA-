...
- `POST /api/invoice-applications/:id/triplicate`：主营项目申请人在线生成固定三联单。页面允许填写工程项目名称和本次实际付款金额；服务端锁定合同链，读取当前有效合同金额，以“已开票金额＋尚未被正式发票覆盖的有效申请余额”作为之前累计，按分计算“之前累计＋本次＝本次累计”，校验本次金额不超过剩余可申请额度且累计不超过合同金额，并生成第一联、第二联、第三联三页 A4 PDF（便携式文档格式）。回款只作为财务佐证，不与正式发票重复相加。
...
- `DELETE /api/invoice-applications/:id?expectedVersion=...`：仅普通员工本人可删除状态仍为草稿且版本一致的开票申请。服务端在根合同和申请行锁内再次校验，不允许删除已提交、驳回或已产生正式发票分配的记录；事务删除材料、生成文件、草稿审计和申请记录，提交成功后再清理对应物理文件。
