...
## 大额报销取消金额门槛（2026-09-22，已上线）

大额报销创建、更新和提交接口不再执行1000元最低金额校验；个人报销列表也不再添加大额报销金额过滤，因此低于1000元的合法大额报销可以正常提交并持续显示。通用发票金额、重复票、购买方、核减、权限和状态校验保持不变，无需数据库迁移或历史数据回填。

## 开放月报福利分类实时联动（2026-09-17，已上线）

读取开放状态月报时，接口继续使用已保存自动快照中的精确金额，但会实时读取福利1、福利2分类主数据，并按稳定分类编号覆盖名称、排序和启停状态。管理员在报销管理中启用、停用、改名或调整分类顺序后，重新进入月报即可看到最新状态，不再要求执行“同步自动数据”，也不会因此重算金额、增加月报版本或写入业务数据。主数据中新出现的分类以0金额加入开放月报；已删除但旧快照仍有非零金额的分类继续以停用历史项保留。已月结报表仍直接返回不可变版本快照，不叠加实时目录。

生产最小发布仅替换月报服务和月报路由两个编译文件，不涉及数据库迁移或前端资源。正式只读接口确认2026年9月仍为工作版本1，407-AI、8H-AI均返回启用且金额保持0；2026年8月仍为已月结版本24。上线前后及正式接口复核后126张业务表内容摘要一致。

## 董事长免审批与福利1、福利2接口（2026-09-09，已上线）

董事长提交基础、大额、商务、福利1或福利2报销时，服务端直接把报销和审批实例写为待付款，并记录 `auto_approved（自动免审批）` 审计动作；普通账号仍走原审批路径。福利1、福利2只允许董事长创建，均必须携带本类型当前启用的分类编号；服务端在事务内读取分类并保存名称快照，客户端不能跨福利账户冒用分类。审批中心、待付款、付款记录和全部查询均返回统一的福利分类编号、名称快照及免审批标记，人工通过和驳回接口明确拒绝两类福利报销。

福利分类接口分别位于 `/api/reimbursement-scope/welfare-one` 和 `/api/reimbursement-scope/welfare-two`，均提供启用列表、管理列表、新增、编辑、排序和删除。删除前在事务内锁定分类并检查报销与月报手工项目：未引用分类可物理删除并写入删除标记，避免默认种子在服务重启时恢复；已有任一引用时返回 `409` 及对应 `WELFARE_ONE_CATEGORY_IN_USE` 或 `WELFARE_TWO_CATEGORY_IN_USE`，历史名称快照保持不变，管理员可改用编辑接口将其停用。`GET/PUT /api/users/me/payment-profile` 供本人读取和保存收款资料。报销付款、付款回单及整包银行回单写接口采用管理员／超级管理员精确角色校验。

## 一般账户五类统计总金额（2026-09-09，已上线）

出账模块新增`categoryTotal`（五类统计总金额）序列并置于`chartMetricKeys`（图表指标白名单）首位；`values`（金额数组）保存同期间已知分类精确和，`partial`（完整性标记）逐期标记完整性。分类序列新增`knownValues`（已知金额数组）保留季度／年度中已知月份的小计，避免某月未知导致其他已知金额丢失。原银行`total`（银行实际总支出）、现金`knownTotal`（现金已录入合计）及其他模块不变；新总金额不进入`breakdown`（饼图分类构成）重复计算。

生产发布只替换`monthlyFinancialAnalysis.js`（月度财务分析服务）及配套活动页面脚本，不新增接口、数据库写入或启动迁移。正式1—9月已知分类总金额906,335.52元，月度、季度、年度、同期和导出均使用原始十进制文本；税费未知未补零，第三季度手续费保留8月已确认的9.00元。

## 三处报销共用来源（2026-09-08，已上线）

确认报销来源扩展事项、类别、范围、完整级联路径、根级行政区和服务对象，继续限定未删除且状态为已审批／已付款／付款凭证已上传／已完成的基础、大额、商务报销。三处共用报销月份、原报销总金额及来源编号；同编号先去重，事实冲突拒绝计算，归属冲突保留未知。行政＝基础＋大额，商务统计＝人力商务，现金账户总额仍按银行交易日期，不修改原记录或月结快照。

## 一般账户出账分类调整（2026-09-08，已上线）

出账模块新增可选`chartMetricKeys`（折线指标白名单）响应字段；正式新响应固定五分类，其他模块未变。行政支出直接合并基础／大额报销来源指标，手续费仅一般账户有效跨行手续费，业务分类不再输出。分类按独立来源核验，账户合计差额保留提示但不抹去已核验分项；权威总额及已录入总额继续保留用于核对，不作为下拉可选指标。接口读取不补原始付款日期；未归期基础／大额报销明确标部分范围。

## 住房分摊最新口径（2026-09-07，已上线）

分析来源指定`housingCostBasis: invoice-lease`（发票全租期分摊）。读取全部未冲正的已确认住房合同发票，包括纯租金及物业票；不再要求收费单月份，不按开票月份或银行付款关联截断发票池。按完整实际租期逐合同计算，累计精确应摊额差确定月份金额；本期、同比、年度及导出沿用同一发票池与租期。可退押金排除，重复／冲突／明细未闭合继续校验，不新增业务写入接口。
...
```typescript
GET    /api/daily-logs/team                                // 获取指定日期团队日志、实时填写状态及完整周统计
...
POST   /api/contracts/:id/auxiliary-packages                    // 辅助合同、发票、回单或其他材料任选一类即可首次归档；单包最多20份
POST   /api/contracts/:id/auxiliary-packages/:packageId/files   // 向已有档案追加任意类型的辅助材料
...
DELETE /api/contracts/:id/auxiliary-packages/:packageId         // 管理员组按版本删除整个辅助档案包，含历史导入档案
DELETE /api/contracts/:id/auxiliary-packages/:packageId/files/:fileId // 管理员组按档案版本单独删除一份辅助材料
...
GET    /api/contracts/:id/deposit                              // 读取租赁押金、逐笔结算及两类退回回单
PUT    /api/contracts/:id/deposit                              // 保存租赁押金金额、条款、资金来源和付款关联
POST   /api/contracts/:id/deposit/return-receipts/recognize    // 多段表单上传押金退款／退工程回单并返回专用识别任务
DELETE /api/contracts/:id/deposit/return-receipts/:jobId       // 删除尚未确认的专用识别任务及物理原件
POST   /api/contracts/:id/deposit/settlements                  // 退款只提交 type=refund、ocrJobId、note；扣款／抵租仍提交金额和日期
POST   /api/contracts/:id/deposit/settlements/:settlementId/refund-receipt // 以 ocrJobId 为历史无回单退款补充已识别原件
POST   /api/contracts/:id/deposit/settlements/:settlementId/engineering-return // 只提交 ocrJobId、note 确认实际退工程
DELETE /api/contracts/:id/deposit/settlements/:settlementId/receipts/:receiptId // 删除错误回单；退款回单有退工程下游时阻断
GET    /api/contracts/:id/deposit/settlements/:settlementId/receipts/:receiptId/file // 财务角色受控在线预览退回回单
```
...
GET  /api/contract-download-requests?scope=admin_pending                 // 目标管理员查看本人任务；超级管理员查看可认领或本人已认领任务
GET  /api/contract-download-requests?scope=admin_processed               // 管理员或超级管理员分页查看本人已经办结的下载处理历史
GET  /api/contract-download-requests/manager-processed/export            // 当前实际审批总经理按审批记录关键词导出本人可见Excel（电子表格）
GET  /api/contract-download-requests/admin-processed/export              // 当前实际执行管理员或超级管理员按处理记录关键词导出本人可见Excel（电子表格）
GET  /api/contract-download-requests/pending-counts                      // 按当前角色读取员工未读结果／总经理待审批／管理员或超级管理员待执行数量
POST /api/contract-download-requests/notifications/acknowledge           // 普通员工以 requestIds（申请编号数组）确认当前页结果已读，每次 1 至 100 条
GET  /api/contract-download-requests/:id                                 // 按申请人、目标总经理、目标管理员或可代执行超级管理员读取详情
...
GET  /api/contract-download-requests/:id/files/:requestFileId/preview    // 申请人、目标总经理、目标管理员或可代执行超级管理员受控在线预览选定文件
GET  /api/contract-download-requests/:id/files/:requestFileId/download   // 管理员或超级管理员认领后下载申请中指定的文件快照
POST /api/contract-download-requests/:id/complete                        // 全部指定文件成功传输后由实际执行管理员或超级管理员标记已处理
```
...
- 总经理目标账号在提交事务中按唯一活动 `general_manager`（总经理）解析；执行目标仍按唯一活动 `admin`（管理员）账号形成姓名快照。目标管理员只能处理分配给本人的任务；`super_admin`（超级管理员）可认领该管理员名下尚未认领的任务。任一账号开始下载后，系统以真实账号写入 `executor_id`（实际执行人编号），其他管理员或超级管理员均不能抢占；董事长及其他角色不能执行。管理员和超级管理员只负责执行，不形成第二次审批。
- 提醒计数不接受客户端传入人员编号：普通员工只统计本人 `updated_at > applicant_seen_at` 的审批／执行结果，其中总经理驳回明确计入未读；总经理只统计分配给本人的待审批申请；目标管理员统计分配给本人的已批准任务和本人已认领的执行中任务，超级管理员统计尚未认领且可代执行的已批准任务及本人已认领的执行中任务，已由其他账号认领的任务不计入可操作待办。员工确认已读必须提交 1 至 100 个 `requestIds`（申请编号数组），服务端去重后仅推进当前员工、指定编号、非待审批且仍未读记录的 `applicant_seen_at`（申请人查看时间），不改业务状态或版本；缺少编号不能批量清除，伪造他人编号不会更新，未翻到的分页结果继续保留未读。
...
- `admin_processed`（管理员已处理范围）允许 `admin`（管理员）和 `super_admin`（超级管理员）调用，按当前账号匹配 `executor_id`（实际执行人编号）且只返回 `completed`（已完成）记录；分页总数与列表使用相同权限条件，并按 `completed_at`（完成时间）、`updated_at`（更新时间）和申请编号倒序。任何账号均不能读取另一执行人的处理历史。
- 目标管理员读取未认领待办详情时必须匹配本人 `target_executor_id`（目标执行人编号）；超级管理员可读取并认领该唯一管理员名下尚未认领的已批准任务。任务进入执行中后，仅 `executor_id`（实际执行人编号）对应账号可继续查看、下载、记录传输和办结，管理员与超级管理员均不能抢占另一账号已认领的任务。已完成详情和重提链同样按本人实际执行编号过滤，不暴露其他执行人的历史或后续尝试。
...
- `GET /api/contracts`（读取合同台账）的 `summary.pendingApprovalCount`（待审批合同数）和 `summary.pendingSealCount`（待盖章合同数）按 `COALESCE(root_contract_id, id)`（主合同链编号）去重，与列表 `total`（合同链总数）使用同一单位。两项快捷指标沿用关键字、分类、项目、区域、日期等台账条件，但计算时忽略当前生命周期状态条件，使审批中与待盖章两张卡在相互切换时继续显示真实数量；列表数据本身仍严格应用请求中的 `status`（状态）筛选；
...
- `GET /api/contracts/:id` 对通过合同区域读取校验的普通员工返回同根合同真实财务数组、对应关系和核算结果，用于只读展示有效回单、未回款、完成率与核算基数；“全部”区域仍禁止普通员工读取，财务新增、确认、删除和冲正接口仍保持财务角色门禁；
...
- 明确项目名称字段首段以项目、工程、变电站、线路或电缆隧道等强主体闭合，且同页下一有效行完整为“前期手续技术咨询服务”等受限尾词时，识别结果返回两行合并后的完整名称与两行证据；“服务内容：”标签、普通正文句、非闭合前期描述和跨页内容不参与该续行。第十八版详情只对已存逐行证据能证明存在此类遗漏、且当前最终值仍等于锚点首段的旧成功草稿返回自动刷新标记；
...
- `GET /:id` 的主合同及每条关联合同必须返回四类合法 `relationType` 或 `relation_type`。前端逐项执行枚举校验；字段缺失、值非法，或新旧命名字段同时存在但值不一致时，整份详情解析失败并进入页面错误态，不渲染合同详情、不恢复草稿，也不得静默回退为独立合同。管理员组读取主合同详情时额外返回 `invoiceApplicationMaterialGroups`（开票申请材料分组），仅包含非草稿、非驳回申请的上传材料、系统生成三联单和当前盖章后材料；其他角色返回空数组；
...
- `PATCH /:id/auxiliary-material-setting` 仅接受布尔值和当前合同版本，使用行锁更新 `requires_auxiliary_materials` 并写审计；已拒绝或已终止合同不能修改。辅助材料档案上传只允许关联已开启该开关的未删除合同：`contract`（辅助合同）、`invoice`（辅助发票）、`receipt`（辅助回单）或 `other`（其他材料）任选一份即可首次归档，后续可追加任意类型。四类文件单包合计最多 20 份，单个文件最多 30MB；支持 PDF（便携式文档格式）、DOC（旧版文档格式）、DOCX（开放 XML 文档格式）、XLSX（开放 XML 电子表格格式）、JPG／JPEG（联合图像专家组格式）、PNG（便携式网络图形）和 ZIP（压缩包格式）。同包相同 SHA-256（安全散列算法）摘要固定拒绝，文件头、扩展名、结构、解压规模或路径任一校验失败时本批已落盘文件全部清理。辅助发票和回单不写入正式财务表，所有辅助材料固定不参与收入、支出、合同额或项目金额核算。整包及单文件删除均使用版本门禁并保留主合同审计；
...
- 合同创建时按锁定大类与非主营收支二级分类直接写入财务方向和来源：主营、非主营收入及其他服务为 `income`（收入），资产类和非主营支出为 `cost`（支出），补充／终止协议继承主合同方向。识别任务只有同时满足 `validationStatus=verified`、`canAutoPost=true`、`documentStatus=normal` 及凭证方向符合合同类型时才返回 `canCreateDraft=true`：收入登记必须配对销项发票与收款回单，支出登记必须配对进项发票与付款凭证。发票业务快照和页面只返回购买方名称、销售方名称、开票名称、发票号码、开票日期、税费、开票金额七项；开票名称完整保留票面内容，例如 `*生产生活服务*技术咨询费`，底层兼容字段仍为 `itemName`（开票名称字段）。双方纳税人识别号及红字／作废判断只作为服务端安全证据，不作为页面识别字段。银行回单只输出并要求电子回单号码、付款时间、付款人户名及账号、收款人户名及账号、金额七项；付款时间保留原件精度，原件只有日期时不伪造时分秒，且不得使用记账日期、上传日期或当前日期兜底。银行凭证的新任务不再要求 Tesseract（开源文字识别）独立证据，但仍要求实际模型为第六版中型模型、单页真实文件、最低行级置信、正常状态及双方账号不同；付款人为公司法定全称时判为付款，收款人为公司法定全称时判为回款，双方同时命中或均未命中时保持方向未知并安全隔离；
- 银行回单整图漏识别付款人或收款人主体时，服务端只使用同一 `PP-OCRv6_medium`（第六版中型模型）对已定位的户名、付款／收款角色和账号区域执行一次受限增强复扫；复扫只补目标主体，不改写金额、日期、电子回单号码或方向。双列表格按坐标隔离两方，角色版式按标签与同一单元格账号绑定，长主体仅在所属单元格内跨行；唯一付款账号不得复制、回退或猜测为收款账号。主体或账号缺失、相同、跨列、多值、归属冲突以及低置信结果继续返回 `blocked`（已阻断）。合同银行回单解析版本升级为 `contract-bank-receipt-parser-v12`（合同银行回单解析第十二版）：币种只接受正文明确的“人民币／CNY／RMB”或人民币货币符号，并记录受控证据类型；仅有“元”、文件名币种、模糊字符、外币或无币种均阻断。旧解析版本且尚未消费的任务在继续办理时自动升级重跑，不复用旧主体、账号或币种结果；
...
- 策略切换不新增或改变路由地址。已消费、已确认、已冲正或已生成正式财务记录的旧双通道任务保持当时结果；尚未生成正式财务记录且识别方法／引擎／解析策略仍为旧版的银行回单，无论旧状态为 `blocked`（已阻断）还是 `verified`（已验证），待登记列表返回 `requiresRefresh`（需要更新）后，由页面按顺序自动调用既有重试接口升级重跑。只读查询本身不改写任务；重跑通过独立写接口在摘要事务锁内递增重试次数、轮换工作令牌并记录旧状态、旧方法、旧版本、证据摘要和阻断码。处理中任务、已消费任务、已有正式记录任务或令牌不匹配的旧实例不得覆盖当前结果；
...
- 工程咨询划拨科技模式的页面操作与接口固定一一对应：当前没有开放登记且本次仅有科技对外付款时，按钮显示“先保存科技对外付款”，调用 `POST /api/contracts/:id/financial-registrations/external-payments` 创建开放登记；已有开放登记后追加科技对外付款时，按钮显示“保存科技公司最终对外付款”，调用 `POST /api/contracts/:id/financial-registrations/:registrationId/external-payments`；开放登记已经保存科技对外付款、当前只补发票时，按钮显示“保存补充发票”，调用 `POST /api/contracts/:id/financial-registrations/:registrationId/settlements`，其中 `bankOcrJobIds`（银行回单识别任务编号数组）必须为空。三条路径不得互相串用；服务端继续反向校验任务角色并在错误路由下拒绝消费任务；
- 旧未消费财务识别任务自动升级重跑后，页面必须使用刷新完成结果中的最终付款角色重新归入“工程咨询→科技划拨回单”或“科技→合同对方付款回单”，随后重新计算按钮文案、开放登记目标和提交接口；不得沿用刷新前卡片类型、旧收款人推断或旧按钮来源。刷新失败、付款角色不唯一或存在多笔未明确选择的开放登记时，不调用任何登记接口；
...
- 社保、公积金银行回单支持同月份、同类别多批次追加，每次仍可批量提交多个文件；接口返回的回单列表与汇总金额覆盖该月份全部已上传文件，按类别累计有效已识别金额。同月份、同类别相同文件摘要继续去重。个税、实发工资仍保留已有文件时后续上传返回 409 的原规则，个人个税明细接口不受此限制。
...
GET  /api/probation/signature-tasks       // 当前账号按真实职责读取待签任务；超级管理员合并主管与原人事部任务
GET  /api/probation/signatures/:id/image  // 有流程权限的账号在原模板中回显签名图片
POST /api/probation/:id/sign-review       // 当前签署人携带预期环节／版本，使用本人签名通过或驳回
GET  /api/probation/list                  // 普通管理员、超级管理员、总经理或董事长按职责读取转正管理列表
GET  /api/probation/:id                   // 普通管理员、超级管理员、总经理或董事长读取申请详情和当前版本签名记录
...
- 人力资源区统一审批工作台只向数据库实时角色为 `general_manager`（总经理）或 `super_admin`（超级管理员）的账号开放；普通管理员继续在员工数据处理人事部意见，董事长继续在员工数据处理最终审批，二者不会因通用管理员能力进入总经理工作台
- 总经理只读取 `supervisor_id`（主管账号编号）为本人的主管领导任务；超级管理员可读取主管账号仍为活动总经理的全部主管领导任务，并继续读取 `hr_approver_id`（人事审批人编号）为本人或尚未指定人事审批人的原人事部任务。申请人本人始终排除，老板、董事长和超级管理员等系统专用账号仍不成为转正申请对象
...
- 超级管理员处理主管领导或人事部环节时只能使用本人锁定签名；`supervisor_id`（主管账号编号）及原审批人姓名快照不改写，签署记录和申请单保存实际超级管理员的姓名、角色与签名，不伪装成总经理或普通管理员
- 超级管理员不能处理 `general_manager`（董事长最终审批）环节；该环节仍只允许申请版本中保存的董事长账号，缺失快照时也只允许真实董事长
- 新版审批工作台提交 `expectedStage`（预期审批环节）和 `expectedFormVersion`（预期表单版本）；超级管理员请求缺少任一值即拒绝，其他客户端只要携带也执行相同校验。服务端在数据库事务锁定申请后比较当前环节与版本，旧页面、重复点击或并发推进产生的过期请求不会写入签名、审批记录或流程状态
...
- 总经理使用 `reviewedByMeThisMonth=1` 查询本人本月实际签署过主管领导意见的申请；超级管理员使用同一参数合并本人实际签署的主管领导意见与人事部意见，普通管理员只查询本人人事部意见，董事长只查询本人最终审批；通过和驳回都计入
- `GET /api/approval/pending-counts` 中，总经理的 `probationSignaturePending`（转正待签数量）与 `probationPending`（转正审批中心数量）只统计分配给本人的主管任务；超级管理员的两项计数均为“活动总经理名下主管任务＋本人原人事部任务”的去重合计；普通管理员只统计本人人事部任务，董事长只统计本人最终审批任务。所有角色均排除本人申请，单条申请当前只有一个审批环节，不会因超级管理员同时具备两类职责而重复计数
...
POST   /api/leave/requests                      // 提交普通请假或单一返岗补假
POST   /api/leave/requests/combined             // 原子提交组合请假或组合返岗补假
...
POST   /api/leave/requests/:id/resubmit         // 草稿提交申请或驳回申请重新提交
DELETE /api/leave/requests/:id                  // 永久删除本人草稿及其完整历史链
GET    /api/leave/pending                       // 总经理、超级管理员或董事长按职责获取待审批请假
GET    /api/leave/reviewed                      // 总经理、超级管理员或董事长获取职责范围内的审批记录
GET    /api/leave/employee-statistics?year=YYYY // 总经理或超级管理员获取年度／月度员工请假统计
POST   /api/leave/requests/:id/approve          // 总经理、受控代办超级管理员或董事长审批通过
POST   /api/leave/requests/:id/reject           // 总经理、受控代办超级管理员或董事长审批驳回
...
POST   /api/leave/admin/requests/:id/cc/mark-read // 当前抄送接收人按通知版本标记已读
...
- 年假、带薪事假、丧假、婚假、产假和陪产假强制启用余额检查；管理员只能调整其默认天数或员工个人年度总天数，不能改为不限额度
- 病假的余额检查可自由开启或关闭，更新接口按请求值保存，不在服务端强制改写；开启后新申请检查并预占年度余额，关闭后按不限额度处理，三甲医院病历或假条的附件校验保持不变
- `PUT /api/leave/admin/balances/:targetUserId/:typeCode/:year` 使用行锁校验调整值不得低于已使用与审批中之和；不限额度类型拒绝余额调整

**请假提交与余额规则：**

- 日期与上午、下午时段由后端校验：普通请假开始日期不得早于服务器当天；返岗补假允许今天或过去日期，但开始、结束日期均不得晚于服务器当天；结束时间不得早于开始时间，单次区间最长 366 个自然日
...
- `requests` 和 `requests/combined` 接收可选 `applicationKind`（申请业务类型）；单一仅允许 `normal` 或 `supplement`，组合仅允许 `combined` 或 `supplement`。`supplement` 在这两个无父申请接口中表示独立返岗补假，非法值直接拒绝
- `requests/combined` 接收总时间段和 2 至 8 个类型分段，按工作半天顺序拆成独立申请；组合返岗补假的各分段统一保存为 `supplement`，共享 `combination_group_id` 且 `parent_request_id` 为空；所有分段在同一事务中校验类型、性别、事由、附件、重叠与余额，任一失败整组回滚
...
- 续假开始时间由上下文接口确定并在表单锁定，必须等于主申请或上一条有效续假之后的首个工作半天；关联追加补假可提交历史日期但必须位于主申请结束之后；组合续假分段被驳回后重提时保持原分段开始时间
- 关联续假和追加补假通过 `parent_request_id` 关联主申请；组合关联申请的各分段保留 `extension` 或 `supplement` 业务类型并共享 `combination_group_id`。独立返岗补假同样使用 `supplement`，但父申请为空
- 待审批、审批历史和申请详情会直接返回主申请的假期类型、起止时间、天数和事由，审批人无需凭申请编号另行查找
- 跨年申请按实际工作日拆分到各自然年，并把年度分配快照保存在申请记录中
- 首次提交、草稿再次提交和驳回后重新提交都会在事务中锁定年度余额后预占审批中天数，防止两个并发申请同时透支
...
- 待办、审批记录、通过和驳回接口只向精确角色 `general_manager`（总经理）、`super_admin`（超级管理员）和 `chairman`（董事长）开放；普通管理员仍只有抄送、配置和余额管理权限
- 总经理和董事长只能处理 `approver_id`（审批人编号）等于本人且申请人不是本人的申请；超级管理员只可代办原审批人仍为活动总经理、申请人角色不是总经理且申请人不是当前超级管理员的待审批申请。总经理本人请假仍由申请中指定的董事长处理，超级管理员不能代董事长审批
- 超级管理员代办不会改写 `leave_requests.approver_id`（申请原审批人编号）；通过或驳回日志以实际超级管理员写入 `operator_id`（操作人编号）、姓名、动作和时间。详情据此显示真实操作人，不能把超级管理员伪装为原总经理
- `GET /api/approval/pending-counts` 中，总经理和董事长的 `leaveApprovalPending`（请假待审批数量）仍只统计分配给本人且申请人不是本人的记录；超级管理员按上述活动总经理代办范围统计；普通管理员固定为 0。计数条件与 `/api/leave/pending`（请假待办）列表一致，不把需董事长审批的总经理请假或当前超级管理员本人申请计入
...
- `GET /api/approval/pending-counts` 新增 `leaveCcUnread`（本人请假抄送未读数量），仅数据库实时角色为 `admin`（管理员）或 `super_admin`（超级管理员）时查询，其余角色固定返回 0；只统计当前接收人的非草稿最新申请版本，与 `leaveApprovalPending`（请假待审批数量）独立。
- 新提交的普通、组合、返岗补假、续假、追加补假、草稿重提及驳回重提，以及通过／驳回事件，在业务事务内为活动管理员与超级管理员生成逐人通知；申请人不通知自己，执行审批的接收人对本人这次审批事件自动已读。旧历史记录不回填提醒，撤回草稿和已有后续申请版本的旧记录不计入未读。
- 抄送列表和申请详情返回当前接收人的 `cc_notice_unread`（抄送是否未读）与 `cc_notice_revision`（抄送通知版本，可为空）；详情在同一数据库快照中读取申请内容与通知版本，避免并发审批导致旧内容匹配新通知。`GET /api/leave/admin/requests?unreadOnly=true` 在数据库分页前筛选本人有效未读，分页总数与列表同口径。
- `POST /api/leave/admin/requests/:id/cc/mark-read` 只允许精确管理员或超级管理员，请求体为 `{ revision }`（已查看的通知版本）；只标记当前登录人、当前申请且版本相符的通知，并返回 `data.unreadCount`（剩余未读数）。客户端不能指定他人账号；过期版本不会清除更新后的提醒，详情加载失败不调用已读接口。
- `GET /api/leave/reviewed` 按申请链只返回当前最终版本，同一申请经历驳回、重提和通过后只占一条记录；当前版本为审批中或草稿时不进入审批记录，撤回及永久删除后也不再展示。超级管理员可查看本人实际通过／驳回的记录，以及当前活动总经理名下非总经理申请人的已处理和历史导入记录；原总经理可查看本人实际处理、分配给本人的历史导入，以及超级管理员代办后写入本人原审批范围的记录。详情接口仍返回实际操作人和完整审批日志
- `GET /api/leave/employee-statistics?year=YYYY` 只允许精确角色总经理和超级管理员读取，不接受客户端指定申请人或审批人，董事长和普通管理员不放行。总经理的已批准叶子范围仍为本人申请，或分配给本人且申请人不是本人、当前角色不是总经理的申请；本人请假即使由董事长审批也按申请人身份纳入，其他总经理不混入
- 总经理本人始终进入目标年及各比较年的员工主集；主总经理再合并全部活动 `user`（普通员工）和 `admin`（管理员），其他总经理只合并本人及历史明确分配的非总经理申请人。超级管理员的统一统计主集为全部活动总经理、活动普通员工和活动管理员；申请事实覆盖全部活动总经理本人的已批准申请，以及分配给这些活动总经理的非总经理申请。超级管理员本人不作为员工被无条件补零，只有本人确有已批准且分配给活动总经理的请假事实时，才作为历史匹配人员纳入，即“有事实纳入、无事实不展示”
- 响应中的 `manager`（统计查看人）返回当前登录总经理或超级管理员的编号、姓名和真实角色名称；`annualComparison`（年度对比）按时间升序返回目标年度及此前四年的统计范围人数、已批准次数和已批准天数，年份不足 2000 时按实际可用年度缩短；人数用于说明该点统计范围，不作为历史在职人数同比
- 每个 `employees`（员工统计）条目返回 `annualTrend`（员工年度趋势）和固定 12 项的 `monthlyTrend`（员工月度趋势）；`monthlyTrend` 每项包含 `month`（月份）、`approvedRequestCount`（已批准次数）、`approvedLeaveDays`（已批准天数）和 `typeSummaries`（假期类型构成）。顶层 `monthlyComparison`（月度对比）同样按 1 至 12 月返回，无数据月份为零值
- 统计范围内的总经理与其他员工使用相同统计结构；前端在部门或姓名筛选后，基于同一响应重新聚合摘要、年度／月度趋势、月度点选下钻、全量员工排名、假期类型分布和逐笔明细，不因筛选发起追加请求；点选当前年或月直接复用已有响应，点选其他年度时按需读取并缓存该年统计
- 年度统计中的带薪假总额、已使用、审批中和剩余四项只读汇总所选年份当前启用且 `requires_balance_check`（需要余额检查）为真的类型；每个员工的 `balanceSummaries`（逐类型余额明细）覆盖全部适用的当前启用类型，额度类型返回总额、已用、待审和剩余，不限额度类型返回 `unlimited=true` 且天数字段为 `null`（空值）；已有记录使用 `leave_balances`（假期余额表），缺失记录仅在响应中按默认额度和年假工龄规则计算，不新增或修改余额行
- 全部请假次数和天数只统计所选年份内状态为已批准的申请版本链叶子；相同 `combination_group_id`（组合批次编号）的最新分段在年度次数中合并为一次，在每个月份内也分别去重为一次，天数仍汇总全部分段。跨月申请先按节假日和调班表展开工作半天；存在有效年度分配快照时，月度合计必须与快照天数守恒，当前日历半天过多按时间截取，不足则从申请区间内未选中的日历半天按时间补足；无有效快照时才完全使用当前日历结果归入各自自然月
- 跨年申请优先读取 `balance_allocations_json`（年度余额分配快照）中所选年份的天数，因此同一申请只把属于该年的工作日计入年度总天数和明细；统计结果按已批准次数降序、已批准天数降序排列，并返回可展开核对的员工逐次申请明细
- 五年对比只执行一次覆盖完整年度区间的已批准叶子申请查询，再在内存按年度展开；同批申请的最早至最晚日期范围至多执行一次节假日查询，所得数据只用于缺失或无效年度分配快照的回退计算，不逐申请读取节假日。员工、余额和档案同样批量读取，整个统计接口不插入、更新或删除任何业务数据
- 普通管理员通过 `/api/leave/admin/requests` 以抄送身份查看全员请假记录、附件和审批进度，不具备通过或驳回权限；超级管理员继续保留同一抄送、假期类型和余额管理能力，并仅在精确角色工作台内增加上述受控代办，不扩张普通管理员权限
...
- 普通组合请假、组合返岗补假、组合续假和组合追加补假的全部最新分段均审批通过后，待审批历史和管理员抄送接口按整组最早开始、最晚结束及合计工作日返回统一返岗信息；仍有待审批、驳回或取消分段时不把该分段提前计入返岗时间
- 审批中的申请撤回后直接变为 `draft`（草稿），立即释放预占余额；草稿再次提交时沿用原编号并重新校验日期、时间重叠、余额、附件和当前审批人，审批日志动作写为 `submit`（提交申请）；驳回后创建新版本的重提仍写 `resubmit`（重新提交）。启动迁移仅将同一申请编号内时间严格递增且紧邻 `cancel`（撤回）的历史 `resubmit` 幂等修正为 `submit`；待审批时间线从最后一次已完成撤回周期之后开始展示
- `requests/:id/resubmit` 可接收 `removedAttachmentIds`（待删除附件编号数组）；仅本人当前 `draft`（草稿）记录直接归属的附件可删除，祖先版本、组合兄弟、其他申请和非法编号均整单拒绝。待删附件行、草稿更新、新附件及重提日志在同一事务提交；事务成功后仅清理无其他数据库引用且位于受控请假附件目录的物理文件
- 请假详情原样返回审批日志的 `historical_import`（历史导入）动作；前端将操作人统一显示为“系统导入”、动作显示为“历史导入”，不展示具体管理员身份，并把该申请的历史证明材料展示在同一审批节点下。历史导入申请使用 `supplement`（补假）且不设置父申请，在各列表和详情中统一显示为“返岗补假”
- 总经理 `GET /api/leave/reviewed`（审批记录）除本人实际执行的通过／驳回和分配给本人的历史导入外，还返回超级管理员代办后仍属于本人原审批范围的记录；超级管理员返回本人实际执行记录及活动总经理范围内的已处理／历史导入记录。列表继续显示真实处理状态和实际操作人，其他总经理、普通导入操作账号及范围外记录不会进入审批记录
- 审批记录接口支持可选 `keyword`（关键词），去首尾空格后最多 200 个字符，超长返回 `400`。按员工姓名、部门的申请快照及当前用户资料、事由、申请编号、假期类型名称进行不区分大小写的字面包含检索；空关键词取消筛选，百分号及下划线不作为通配符。总数与分页列表使用相同条件，在全部本人可见的最终版本记录中先筛选后分页，审批时间相同的记录按申请内部编号稳定排序。
- 必传附件类型按“版本链保留附件减去待删附件，加上本次新上传附件”重新校验；删除最后一份证明且未上传替代材料时拒绝重提，取消编辑、校验失败或事务回滚均不会删除旧附件
- 四个请假上传入口统一限制最多 5 个附件、单个不超过 5MB；文件过大返回 `413` 和“单个请假附件不能超过 5MB”，数量或格式错误返回 `400`。上传中间件会清理同批已落盘临时文件，错误不会再进入通用 `500` 处理
...
- 公司主体只出现在销售方时不通过购买方校验；基础、大额、商务、福利1和福利2报销共用该规则
...
| 方法   | 路径                                                        | 权限           | 说明                                                                   |
| ------ | ----------------------------------------------------------- | -------------- | ---------------------------------------------------------------------- |
| `POST` | `/api/bank-receipts/upload`                                 | 管理员         | 上传一份或多份银行回单 PDF（便携式文档格式），整包拆页、切分并异步识别 |
| `GET`  | `/api/bank-receipts/batch/:batchId`                         | 管理员         | 查询整包处理状态、单张回单及其匹配结果                                 |
| `GET`  | `/api/bank-receipts/unmatched`                              | 管理员         | 查询自动复核后仍待认领的单张回单                                       |
| `POST` | `/api/bank-receipts/:id/match`                              | 管理员         | 将待认领回单手工关联到指定报销单                                       |
| `POST` | `/api/reimbursement/payment-batch/:batchId/confirm-receipt` | 当前批次申请人 | 按付款批次一次确认同批全部待确认报销单                                 |

- 整包只负责文件处理和识别调度；自动匹配与手工认领均调用同一“每张回单一个 `transaction（事务）`”持久化入口。服务端在事务内锁定回单、报销单和付款批次，并复核唯一有效的银行实际交易日期、按整数分计算的金额合计、收款人姓名及完整收款账号。
- 回单裁片 SHA-256（安全散列算法）摘要或规范化电子回单号已经使用时返回重复错误。没有付款批次的目标报销单必须已经处于 `paid（已确认付款）`，系统随后自动创建 `uploaded（已上传回单）` 付款批次及明细快照；已有唯一 `pending（待处理）` 批次时，报销单可仍为 `approved（已审批）` 或已转为 `paid（已确认付款）`，但请求成员、快照金额、同一收款人和回单总额必须完全一致。
- 自动候选在状态、金额、收款人或防重复核中失败时，候选写入会先完整回滚，再把同一识别证据原子保存为待认领；数据库或基础设施异常不会伪装成待认领。手工认领只接收回单编号和目标报销单编号，服务端在事务内读取既有 OCR（文字识别）证据；历史记录缺少完整账号、电子回单号或唯一交易日期时失败关闭并要求重新上传原件。
- 匹配成功后，付款批次、批次明细、报销状态、付款回单、防重记录和审批记录共同提交；任一步失败全部回滚。申请人最终通过付款批次确认收款，同批报销单与批次状态在同一事务内完成。

**数据显示规则：**

- `/api/reimbursement/list`：默认显示最近一个月的数据，按创建时间升序排列（早的在下，晚的在上）
- 大额报销的创建、更新、提交和个人列表查询均不设置1000元最低金额门槛；列表只应用通用类型、月份、状态和删除标记条件
...
| 方法 | 路径 | 角色 | 说明 |
| --- | --- | --- | --- |
| `GET` | `/api/monthly-financial-reports/analysis` | `super_admin`、`admin`、`general_manager` | 同连接只读查询七模块分析，独立筛选、期间聚合和精确金额 |
| `GET` | `/api/monthly-financial-reports/analysis/export` | `super_admin`、`admin`、`general_manager` | 核对内容版本后导出单模块或全部分析，金额文本保真 |
| `GET` | `/api/monthly-financial-reports/analysis/projects/:rootId/receipts/:receiptId/preview` | `super_admin`、`admin`、`general_manager` | 校验当前回款、项目期间和证据版本后预览银行回单，不返回裸存储路径 |
| `GET` | `/api/monthly-financial-reports/trend` | `super_admin`、`admin`、`general_manager` | 兼容保留的旧趋势查询；当前七模块页面不再调用 |
| `GET` | `/api/monthly-financial-reports/:month` | `super_admin`、`admin`、`general_manager` | 查询指定自然月的账户、收支、来源、校验和权限 |
| `PUT` | `/api/monthly-financial-reports/:month/manual-items` | `super_admin`、`admin` | 提交完整手工项目列表，服务端按编号差异增删改；系统首月可同时初始化四账户期初 |
| `POST` | `/api/monthly-financial-reports/:month/refresh` | `super_admin`、`admin` | 重新汇总合同回款、工资、已支付报销和资产付款 |
| `POST` | `/api/monthly-financial-reports/:month/close` | `super_admin`、`admin` | 执行月结、保存期末和不可变版本快照 |
| `POST` | `/api/monthly-financial-reports/:month/reopen` | `super_admin`、`admin` | 必填原因后重新开启已月结报表 |
| `GET` | `/api/monthly-financial-reports/:month/export` | `super_admin`、`admin`、`general_manager` | 按《月度结算表》模板下载四工作表 XLSX（电子表格）并记录持久化月报的下载审计 |
| `GET` | `/api/monthly-financial-reports/:month/bank-receipts` | `super_admin`、`admin`、`general_manager` | 查询基本、一般、商务三账户当前文件、分类汇总、异常和单笔回单 |
| `POST` | `/api/monthly-financial-reports/:month/bank-receipts` | `super_admin`、`admin` | 以 `multipart/form-data（多段表单）` 上传一至三份原始银行 PDF（便携式文档），识别并原子刷新月报 |
| `POST` | `/api/monthly-financial-reports/:month/bank-receipt-corrections/:reviewId/confirm` | `super_admin`、`admin` | 重传首次复核的完整原件批次，以一次性令牌、会话绑定、文件摘要、分析摘要、差异确认清单和核实原因确认历史识别纠错 |
| `POST` | `/api/monthly-financial-reports/:month/bank-transactions/:transactionId/review` | `super_admin`、`admin` | 必填原因后复核排除待复核交易，保留原件与审计并重新计算月报 |
| `GET` | `/api/monthly-financial-reports/bank-transactions/:transactionId/preview` | 已登录用户；`super_admin`、`admin`、`general_manager` 可读全部，其他申请人仅可读本人有效报销链接 | 内联预览整月银行原件中裁出的单笔回单 |
| `GET` | `/api/monthly-financial-reports/bank-transactions/:transactionId/reimbursements/:reimbursementId/previous-proof` | 已登录用户；`super_admin`、`admin` 可读全部，`general_manager` 仅可读商务报销，其他申请人仅可读本人报销 | 通过当前有效替换链接内联预览被替换的原付款回单；`index` 指定多文件序号 |

月份必须为 `YYYY-MM`。所有写接口必须携带 `expectedVersion`（预期版本），使用月份级事务锁和可串行化事务；版本不一致、首次并发创建及序列化冲突统一返回 `409` 和 `MONTHLY_FINANCE_VERSION_CONFLICT`。未持久化月份必须先保存或同步，不能直接月结。总经理的任何写请求，以及董事长、BOSS（经营管理）和普通员工不具备权限的月报请求，均在服务端权限中间件处拒绝；超级管理员与管理员同权。

实际税费支付复用既有 `PUT /api/monthly-financial-reports/:month/manual-items`（保存月报手工项目接口），不增加税务接口。项目分类 `general_tax_payment`（一般账户实际税费支付）只允许 `accountCode=general`（一般账户）及 `direction=expense`（支出）；`amount`（金额）必须大于零，最多 18 位整数和 12 位小数，`occurredOn`（发生日期）必须是真实日期且属于请求月份，`description`（业务说明）和 `voucherReference`（凭证号）去除首尾空白后均不可为空。错误账户／方向、跨月日期、无效金额或缺少说明／凭证号均在保存前拒绝；仍采用完整列表、按编号差异保存、版本锁及已月结不可直接修改的原有规则。有效税费进入一般账户流出及分析 `tax`（实际税费分类），没有有效凭证记录的历史月份保持未知，不把主营税费预留转成付款记录。

七模块分析查询参数为 `from`（开始月份）、`to`（结束月份）、`granularity`（月／季／年，分别为 `month`、`quarter`、`year`），以及可选 `partyA`（合同甲方）、`contractRegion`（合同区域）、`reimbursementScope`（报销范围）、`personId`（人员编号）、`comparisonYear`（对比开始年份）。月份限 1900—2099 年、单次最多 240 个月；重复参数数组、非法粒度及超长筛选返回 `400`。对比年份必须是 1900—2098 年间且早于本期开始年份的整数，起止月份按同一年差整体平移，保留同粒度和业务筛选；比较范围也须落在有效月份内，不传或留空即不比较。

统计查询继续按明确的起止月份聚合，不因为选择季度或年度而自动扩成整季／全年。图表历史窗口复用同一分析只读接口：月度请求连续 12 个月，季度请求最多连续 12 个自然季度并保留 `granularity=quarter`（季度粒度），不再全部强制转为月度；默认季度窗口截至统计结束月份，最新季度可按该截止月截尾，1900 年边界不足 12 季时不补造更早期间。前端仅接受完整自然季边界或最新季度明确截止月的窗口事件；这不限制统计接口本身查询不完整季度。年度主图直接使用已显示统计响应，不另建年度历史窗口。窗口浏览、各模块对比显隐均不改变主统计条件或导出版本，只有“用当前窗口统计”才明确提交新的月／季范围；季度窗口的对比开始年份随窗口平移，但保持原同比年差。

当前月度财务报表页面已移除“连续历史趋势与同期对比”旧折叠区，已建月报与空月分支均不再加载旧趋势数据，也不再为该区发起完整历史分段请求。`GET /api/monthly-financial-reports/trend`（旧趋势只读接口）及其类型、服务和工具仅为兼容继续保留，当前七模块使用分析查询／导出接口及月／季分析窗口，不依赖旧趋势接口返回的可用年份。往年对比候选年份覆盖 1900 年至统计开始年份的前一年；候选可选不代表该年存在完整数据，来源缺失仍返回未知，不补造历史金额。旧接口权限、范围限制和金额口径未因页面移除而改变。

本轮分析结果新增或细化以下只读字段与口径，金额仍为精确字符串或 `null`（未知）：

- `outflow.series`（一般账户出账趋势序列）增加 `total`（一般账户总支出），每个月／季／年的值直接聚合权威月报一般账户流出，不以分类合计反推。`breakdown`（分类构成）仍仅含六个互斥支出分类，不包含总额；税费分类未知或分类明细不闭合时，已核验的总支出仍可显示，真正缺月或月报来源不可用时总额仍为未知。
- `projects.details`（项目期间明细）包含 `periodKey`（期间编号）、`from`（期间开始月份）、`to`（期间结束月份），每行对应该期间的一个合同根项目。原 `contract`（合同额）、`received`（累计已回款）、`outstanding`（未回款）继续使用期间末口径；`sourceId`（来源编号）为合同根编号，`region`（行政区）读取根合同的 `area`（行政区字段），不从项目名猜测，也不跨期间重复归集期末金额。仅流量行与新期间回款字段的说明见下文。
- `projects.series`（项目趋势序列）新增 `signedContractCount`（主营项目签订合同数量），`unit`（单位）为 `个`。父序列 `values`（期间值）为当期全部新签主营主合同链数量，`segments`（分组序列）按根合同 `area`（行政区）和 `party_a`（合同甲方，即本指标服务单位）组合返回稳定 `key/label/region/serviceUnit/values`；本期与顶层 `comparison.modules`（往年同期模块）使用相同分组键。只计有效主营根主合同且仅接受 `manual`（人工确认）或 `ocr`（文字识别确认）的真实签订日期；补充、解除不另计，终止主合同保留历史数量。已发生空期为字符串 `0`，未来期或来源不完整为 `null`。前端选择该数量指标时，如响应已包含往年同期序列，应自动显示对应历史对比折线；该行为不改变任何金额类指标的对比显示。
- 每个签订数量分组另返回 `projectNames`（项目名称列表），类型为 `Array<string[] | null>`（按期间排列的项目名称数组或未知值），外层位置与所属模块的 `periods`（期间列表）及该分组的 `values`（期间值）严格一一对应。数量为字符串 `0` 时同位置必须为 `[]`（空名单），数量为 `null`（未知）时必须为 `null`（未知名单）；非零名单条数须与数量一致，同名主营主合同按不同根合同逐条返回，不按名称去重。客户端点选本期／往年同期及月／季／年数据时，必须分别使用对应模块真实 `periods`（期间列表）的索引；历史滑动响应同样按自身真实期间定位，禁止把可见横轴序号直接套到另一期间或另一响应。
- `business.breakdown`（商务区域构成）与 `business.comparison`（商务区域比较）按报销范围的完整父链根级归属分组，公司内部单列。商务明细的 `scope`（范围展示值）在可核验时显示完整路径，`scopePath`（完整路径）、`region`（行政区）可为空，`regionSource`（区域归属来源说明）解释冻结、当前核验或未知原因。同名下级范围不跨区合并，同值多路径、缺父级、循环及编号冲突均归“行政区未知”，不复制业务金额；旧叶名称无法判断是否属于当前完整路径筛选时，筛选合计保持未知而非零。

新自动明细的 `analysis`（冻结分析维度）增加四个可选字符串／空值字段：`reimbursementScopeValue`（报销范围原值）、`reimbursementScopePath`（完整范围路径）、`reimbursementRegion`（行政区）、`reimbursementRegionSource`（区域归属来源说明）。结构版本仍为 1；非空冻结行政区必须同时有原值、路径和归属说明，字段类型、长度及来源类型继续校验。已有冻结字段优先，不用当前配置覆盖历史；旧月结缺这些字段时保留区域未知，不回填旧快照。本次沿用原快照结构，无数据库迁移或新增接口。

项目趋势另增加 `periodReceived`（期间实际回款）序列，月／季／年标签分别为“本月回款”“本季回款”“本年回款”。它直接按当前 `confirmed`（已确认）且未冲正的人民币主营回款凭证及有效业务日期汇总到对应期间，不使用累计余额差分，也不沿用累计指标的历史冲正截止算法；在同一事实集合与范围下，月度值可精确合并为季度或年度值。其 `summaries`（摘要）中的同名指标表示整个已提交 `from/to`（起止月份）范围的实际回款，不是最后一个月或季度的值。旧 `received`（累计已回款）仍按原历史截止算法计算，已月结入账快照也不回写，因此两类口径可能不同。

项目明细同步包含 `periodReceived`（期间实际回款）、`periodReceivedSourceState`（期间回款口径说明）、`periodReceivedSourceIds`（回款来源编号，以顿号连接）。`detailKind`（明细类型）为 `project_period`（原期间项目行）或 `period_receipt_only`（仅流量行）；合法预收款可在原期末算法尚不展示该项目的期间形成仅流量行，其原合同额、累计已回款和未回款均为 `null`（不适用或未知），不进入旧三项序列或旧累计下钻。已完成、已终止合同的真实尾款同样不因合同日期截掉；草稿、驳回、删除等仍遵循原可读取合同根范围。合同签订日期缺失只影响原期末指标，不抹去回款日期和币种已核验的收款事实。

新增期间回款保留异常证据边界：缺可靠业务日期、缺明确人民币币种、来源编号缺失、重复来源编号或同银行规范化回单／流水编号冲突时，受影响值返回未知及说明，不把过滤后的人民币片段当完整金额，也不任意选取重复凭证。异常日期候选保留供诊断而非提前丢弃；有效日期晚于生成日的记录不计入实际发生额，含未来月份的期间及摘要保持未知。项目 `updatedAt`（来源更新时间）纳入关联回款真实更新时间，不以查询生成时间替代。计算字段继续使用现有分析／导出接口和权限；银行回单使用下述独立受控预览入口，不迁移数据库或回写旧快照。

项目明细的 `projectNumber`（展示项目编号）读取根合同 `business_contract_no`（主营合同原件业务编号），并以 `projectNumberSource`（编号来源说明）明确来源。没有可靠原件编号时为空，不拿自动合同流水号或内部定位编号替代。`periodReceiptNumbers`（期间回款凭证编号）只汇总真实银行回单号码；原 `sourceId`（合同内部追溯编号）和 `periodReceivedSourceIds`（回款内部追溯编号）仍保留技术追溯含义，不改标签冒充业务号码。

项目模块增加可选 `projectReceipts`（期间回款凭证清单），类型为 `FinancialAnalysisProjectReceipt`（项目回款凭证数据），不改变通用 `details`（明细）只含字符串／空值的约束。清单按本期／同期各自的合同根和实际期间关联，包含 `rootContractId`（合同根编号）、`receiptId`（回款记录编号）、`periodKey/from/to`（期间编号与起止月份）、`projectNumber/projectNumberSource`（业务编号与来源）、`receiptDate`（回款业务日）、`amount`（精确字符串金额）、`receiptNumber/receiptNumberSource`（真实银行回单号与字段来源）、`transactionSerialNo`（独立交易流水号）、`bankName`（银行名称）、`fileName/mimeType`（文件名与媒体类型）、`previewUrl`（受控预览地址）和 `previewUnavailableReason`（不可预览原因）。回单号优先采用 `electronic_receipt_no`（电子回单号），其次采用真实 `proof_no`（凭证号）；仅有流水号或旧凭证号只是流水号的复制值时，回单号留空。缺文件或关联无法核验不伪造地址，也不改变期间实际回款金额。

新预览入口必须携带 `from`（期间开始月份）、`to`（期间结束月份）、`evidenceVersion`（证据版本摘要）。每次在同一只读事务中复核账号当前有效财务角色、合同根归属、主营且未删除／未驳回状态、回款当前已确认且未冲正、明确人民币币种、真实业务日期在请求期间且不晚于生成日；金额或文件关系与版本摘要不一致时要求刷新。摘要用于拒绝过期证据，不代替权限校验。唯一有效的 `canonical`（规范银行裁片）优先，裁片必须当前有效、识别通过、金额日期与回款一致且关联唯一；已有替换关系冲突、失效或退役而没有有效裁片时，不悄悄回退另一张原件。只有从未存在替换关系时，才可使用回款明确的 `file_id`（原件编号），并要求文件与回款属于同一个合同及同一根链，而非猜选同链兄弟合同附件。

预览仅允许 PDF（便携式文档）及 JPEG／PNG（安全位图），同时核对真实文件头和媒体类型。服务器按证据种类限定合同上传或月报银行上传目录，并以 `realpath`（真实路径解析）检查符号链接后的最终目录；禁止目录逃逸、HTML（网页文档）、SVG（可执行矢量文档）及不明格式。返回 `no-store`（禁止缓存），缺文件或校验失败使用中文说明且不泄漏磁盘路径。现有合同文件与月报银行宽预览接口保持原有用途和权限，本入口不改变它们的历史证据范围。文件元数据只为当前期间、筛选及生成日内可展示的有效回款批量读取，完整历史回款仍保留给累计计算；无数据库迁移、旧快照回写或金额调整。

启用对比后，响应顶层增加 `comparison`（对比期），包含 `label`（说明）、`query`（实际范围和筛选）、`modules`（七模块结果）、`warnings`（对比期提示）。子查询不再携带 `comparisonYear`（对比开始年份），不递归生成第三期；本期和对比期分别保留金额及状态，不混加。顶层对象与单模块原有 `comparison`（柱状比较数据列表）并非同一层级。双期来源读取复用同一可重复读只读事务连接和月报读取缓存，两期各按自身范围加载，不为相隔多年而加载中间无关年份。各模块金额仍为十进制字符串或 `null`（未知），缺失对比数据不得补零。

余额模块的 `title`（展示标题）统一为“账户余额资金台帐”，本期及对比期一致，导出工作表名称同步派生。`balances`（余额模块键）、查询参数、金额字段与数据口径均不变。

其余更名为：`inflow`（入账模块）“一般账户入账统计”、`outflow`（出账模块）“一般账户出账统计”、`projects`（项目模块）“项目分析”、`business`（商务模块）“商务统计”、`personnel`（人力模块）“人力成本分析”；本期、对比期和导出表名一致，接口键及字段结构不变。

2026-09-07 本轮开发版在人力模块增加可选 `housingCostBreakdown`（住房分摊前公司构成），每项包含 `key`（类别键）、`label`（类别名称）、`amount`（完整确认金额）、`knownAmount`（可核验已知金额）及可选 `note`（来源说明）。金额保持精确十进制字符串或 `null`（未知）。该构成受本期／同期各自已提交统计期间约束，单人筛选不改变公司分摊基数；原 `overhead`（分摊费用字段）在新输入下表示同住房合同的完整住房费用分摊，不再仅指租金。可退押金及其他资产合同排除，未增加写入接口或修改现金统计口径。

住房固定租金和物业使用有效房屋租赁的明确月费、租期及当前业务日；变量费用只取该住房合同的已确认发票行，按原票明确收费月份归期。原票文本必须与登记发票号码及完整价税合计一致，票面金额校验不能只用电费／维护小计代替整票；收费月缺失、冲突、原件不可核验或明细不闭合时保留未知，不按文件名、开票日或付款日猜测。混合票里的租金、固定物业不与租期计提重复相加；其他已确认住房费用须有同合同和真实费用依据。各项按月合并后只执行一次年度累计精确尾差分摊，工资和三类报销不重复计入。

人力导出在既有统计／明细／年度表之外，按有无住房构成数据增加 `-住房构成`（独立构成工作表），记录公司分摊前费用、实际统计起止、确认金额、已知金额和说明；同比同样生成其自身构成表。该表只用于核对，不与人员费用表重复求和，所有金额与文本继续以文本单元格保真输出。工作表总数随人力年度、住房构成和未归期报销附表是否存在增加，不应按早期固定表数验收。本轮尚未部署，详见 [住房全成本分摊验收-2026-09-07.md](../docs/住房全成本分摊验收-2026-09-07.md)，旧发布日志保持原历史事实。

分析来源时间仅从合法的业务更新时间、报表保存／月结时间选择，按实际时刻比较并输出标准时间；非法原值不直接显示，不以本次查询时钟替代来源时间，避免没有业务变化却无法同版本导出。

分析返回 `dataVersion`（内容版本），忽略纯查询生成时间但覆盖本期及对比期的稳定内容与筛选变化。导出复用同一加载聚合逻辑，额外接收 `module`（七模块之一或 `all`）与可选 `dataVersion`；页面导出固定携带当前显示版本，任一期变化返回 `409` 和 `MONTHLY_FINANCE_ANALYSIS_CHANGED`。不含人力年度、住房构成及未归期报销等附表的基础结构为：无比较时全部15张、单模块3张；有比较时全部29张、单模块5张，均含一张查询说明及各期统计／明细表。实际工作表数量应加上存在的人力附表，不再以基础数量作为全部导出的固定断言。查询说明记录比较年份和实际范围，对比表使用“对比期”前缀且表名不超过31字；比较模块不完整时拒绝输出缺期文件。金额及来源文本均显式写成文本单元格，防止精度丢失及公式执行；查询、导出均禁止缓存，并在同一只读事务内复核有效角色，不触发月报维护副作用。

工资分项沿用 `/api/payroll`（工资接口）的管理员权限：超级管理员、管理员可见；总经理的人员模块设置 `payrollDetailsVisible=false`（工资分项不可见），移除工资／公司社保／公积金／实际调整的明细列和值以及构成图数据，保留已授权的成本合计。本期 `modules`（模块列表）及顶层 `comparison.modules`（对比期模块列表）使用相同过滤，不修改原来源对象；版本指纹和导出均基于双期过滤后的数据，不能通过往年对比获得受限工资。原月报自动明细新增的 `analysis.payrollParts`（冻结工资分项）也不会返回给总经理。

银行上传按真实文件头再次校验 PDF（便携式文档），单份上限 50MB／120页，本批合计上限200页，一页最多上下两张，原文件不合并。完整账号只允许基本账户 `0200049609201258271`、一般账户 `0200303519000018418`、商务账户 `321240100100245908`；文件名只作为异常提示。文件摘要按“报表月＋摘要＋识别版本”联合查重；同一识别版本的部分重复原件跳过，全部重复原件不新增版本但继续执行报销重匹配。识别规则升级后，同一原件允许生成新的受审计证据版本，旧文件和旧识别值继续保留，用于修复左右栏绑定等历史解析问题。同一混月原件可切换到其他月份重新上传并确认；单一银行交易月份与当前月不一致时返回 `MONTHLY_BANK_REPORT_MONTH_MISMATCH`；文件含多个交易月份且未确认时返回 `MONTHLY_BANK_MIXED_MONTH_CONFIRMATION_REQUIRED`。管理员确认后只生成当前报表月的交易事实，其他月份不跨月写入，页面提示切换到对应月份重新上传并确认。

同月同账户的电子回单号已经存在，但日期、金额或双方账号与本次原件不一致时，服务端先在可串行化事务中锁定月报和回单并返回 `MONTHLY_BANK_RECEIPT_CONTENT_CONFLICT_CONFIRMATION_REQUIRED`，同时创建30分钟有效的一次性复核挑战；此阶段不替换旧文件、不更新交易、不提升月报版本。页面逐笔展示历史值与本次原件值，管理员填写10至500字原因后，确认接口必须重传首次全部非重复原件。服务端重新计算文件摘要和完整分析摘要，并复核管理员、会话、月份、版本、旧事实、解析版本及逐笔差异摘要；完全一致后才原子换版并写 `bank_receipt_correction（银行回单纠错）`审计。存在合同、报销、薪资等有效业务链接，或本公司账号所在收付方向发生变化时，仍返回不可直接纠正的硬冲突。

上传识别结果存在失败页、无法拆分或多回单号内容冲突等 OCR（光学字符识别）不确定性时，接口在进入持久化事务前返回 `MONTHLY_BANK_REVIEW_REQUIRED`，不关闭旧活动文件、不生成新文件版本，也不刷新月报。一般账户中字段完整但备注关键词不足的回单可以继续进入合同严格核对；主营收入与资产支出按一般账户归属、收支方向、电子回单号、双方完整账号、银行交易日期、金额、确认状态和合同分类关联已有合同收付款。没有对应记录或信息不一致时不再整批失败，而是将该笔保存为 `review_required（待复核）`，银行金额不进入经营统计，并把文件标记为 `partial（部分待核对）`。同文件已确认的一般／商务利息和跨行手续费继续计入月报，主营和资产金额仍取系统业务记录。`POST /:month/close` 仍会拒绝部分文件或待复核交易，不能冻结不完整快照。

未月结报表的 `GET /:month` 会在返回按钮权限前实时叠加活动银行文件和待复核交易状态，合同凭证冲正后无需依赖旧自动快照也会立即禁用月结入口。中文上传文件名统一经过字符集还原和安全规范化后再识别、保存及展示。自动数据在月结事务内使用同一连接依次读取，事务外读取仍可并行，避免数据库驱动并发复用单连接的兼容错误。

合同候选读取使用 `FOR SHARE（共享行锁）` 与冲正互斥，禁止“先读已确认、后被冲正、再写活动链接”的竞态。冲正形成的待复核交易可由管理员在重新开启的报表中必填原因后排除；系统同步恢复文件状态、重算自动快照、提升版本并写审计。历史月结明细预览携带当时文件编号，不跟随当前交易裁片换版。浏览器支持时在请求前计算并保存客户端 SHA-256（安全散列算法）摘要；局域网 HTTP（超文本传输协议）环境无法计算时仍发送原件，由服务端摘要、回单号、月报版本与唯一约束防重。网络中断、停止等待或强制刷新后保留不确定状态；有摘要时自动核对，无摘要时刷新后仅允许管理员手动安全重试，页面不自动重发。数据库提交响应不确定时保留原件，不执行可能破坏已提交事实的清理。

所有金额字段以十进制字符串返回，不固定小数位、不默认补零；`totals.income` 取系统内已确认主营合同回款总额，不是税费及账户拆分后的合计。主营回款、资产支出、报销和薪资均以系统业务记录作为统计来源，银行回单只负责匹配、挂载和换版凭证；一般／商务账户利息与跨行手续费例外取逐笔银行回单金额。查询响应通过 `automaticDetails`（自动来源明细）返回人员级工资成本、基础报销、大额报销、商务报销，以及一般／商务物理账户的逐笔利息和跨行手续费。银行控制的四类费用还会作为 `readOnly（只读）` 手工项目行返回回单号与预览地址，旧手工行保留但标记为非生效。银行明细同时返回资金账户代码、物理 `bankAccountCode`（银行账户代码）、电子回单号和预览地址；单笔银行交易保留旧 `linkStatus`（关联状态），并分别返回 `employeeMatch`（员工匹配）与 `reimbursementLink`（报销关联），后者包含有效关联报销、挂载／替换动作、原回单和当前有效裁片地址。普通申请人预览裁片时，服务端必须验证该交易存在指向本人未删除报销的有效展示替换链接；无有效本人链接或裁片路径逃出月报银行上传根目录均返回 `403`。被替换的原付款回单不再通过“当前付款路径”反查，而是由专用接口校验当前有效 `display_replacement（展示替换）` 链接中的 `previousPaymentProofPath`；只允许超级管理员、普通管理员、商务报销总经理或该报销申请人读取，并把可访问文件严格限制在 `uploads/invoices`、`uploads/bank-receipts`、`uploads/monthly-financial-bank` 三个证据根目录，目录穿越、符号链接逃逸和失效链接均拒绝。`bank.chargeAccounts` 用于判断一般、商务手工利息／手续费是否已由银行文件提供；基本账户利息与手续费分类为忽略。下载文件包含“月度结算、手工项目明细、自动来源明细、校验”四个工作表。已月结的查询和下载只读取对应版本快照；快照缺失或结构损坏返回完整性错误，禁止实时重算。历史月重开、早月补建或维护会通过 `affectedMonths` 返回被级联标记为待重新核算的后续月份，旧快照继续保留。完整字段、分类和错误边界见 [16-月度财务报表模块.md](./16-月度财务报表模块.md)。

`POST /:month/bank-receipts` 和 `POST /:month/refresh` 均在各自月报事务内调用报销回单补挂服务。上传响应额外返回 `reimbursementReconciliation`（报销协调结果），包含已挂载组、待关联交易、凭证替换证据和具体月报链接。原件摘要全部已存在时，上传接口不再直接返回重复错误，而是在相同版本、权限及月状态门禁下只执行重匹配并返回“文件已存在，已重新匹配并挂载报销回单”。待关联只表示尚未找到唯一报销，不会伪造批次或占用付款凭证摘要。

月报补挂、审批中心单笔／批量上传和整包银行回单统一规范电子回单号并使用相同文件摘要／回单号事务锁。审批中心接口在事务内重新读取并锁定报销、付款批次和批次明细，更新必须同时命中预期状态及 `payment_batch_id`（付款批次编号）；并发状态变化返回 `409`，不能生成双批次或覆盖另一条路径已提交的回单。事务确定失败会清理本批新落盘文件，提交结果不确定时保留原件并要求刷新确认。
...
- 2026-09-07: 主营项目签订合同数量的每个分组新增 `projectNames`（项目名称列表），与 `periods`（期间列表）及 `values`（期间值）逐项对齐；零值对应空名单、未知对应 `null`（未知），同名主合同不去重，本期／同期及月季年、历史滑动按真实来源期间索引隔离。
- 2026-09-07: `signedContractCount`（主营项目签订合同数量）的响应单位统一为“个”；响应包含往年同期分组序列时，前端选择该指标后自动显示历史对比折线，不改变金额类序列、参数、权限或数据库。
- 2026-09-07: 财务分析项目模块响应新增主营项目签订合同数量父序列及“行政区＋合同甲方”分组序列，单位为“个”；往年同期沿用相同键，导出按整数写入总列和分组列。接口参数、权限、金额序列及数据库保持不变；生产只读接口及发布前后55张业务表指纹核验通过。
- 2026-09-07: 人力租金明细内部新增自然年累计精确尾差分配服务，响应结构不变；月／季／年及个人筛选结果一致，其他六模块和公司月度总额不变。已完成隔离启动、权限、导出及生产验收，未新增写入接口或改业务记录。
- 2026-09-07: 已上线人力专属确认报销月份只读来源和租期发生额，模块返回personnelReimbursementBasis说明统计依据，未归期清单reason明确为报销月份问题；缺付款日期但报销月有效的记录可纳入人力，现金接口不变。工资权限、两期与年度输出及金额闭合验收通过，无新写入接口。
- 2026-09-07: 开发版人力来源新增全历史未归期报销的稳定人员诊断及有效房屋租赁月租／租期只读数据；新增personnelUnassignedReimbursements独立输出与导出，人员完整性不再被无关人员缺日期记录污染。租期发生额不改现金账务，最终归月策略待确认，未上线。
- 2026-09-07: 人力分析年度清单、已知分项及仅房租口径已发布生产，工资权限与双期导出保持；启动和只读接口验证未改业务表，其他六模块结果与基线一致，没有新增写入接口。
- 2026-09-07: 人力分析保留 overhead 字段兼容，含义改为仅房租分摊；来源层明确排除非租金资产，混合票面完整分配后只纳入租金行。月报付款身份、租金日期／币种／完整性检查继续保留，不改其他账户支出或原始业务数据；独立副本核对通过。
- 2026-09-07: 分析接口人力模块新增 personnelAnnualDetails 自然年独立明细，期间与年度行增加 from/to/year 及 known\_分项；导出增加本期／同比年度表，工资敏感分项过滤覆盖年度与已知字段，未新增写入接口或修改数据库。已在独立正式数据副本验收，生产发布等待业务数据修正批准。
- 2026-09-07: 分析来源新增与月报同口径的普通资产付款只读来源，保留两期合并及金额字符串；历史导入回款通过原验证任务快照匹配取得兼容资格，不把普通空币种自动当人民币。商务完整路径筛选支持父级前缀；人员比较保留明确已知部分说明，完整总额不替换，无新写入接口。
- 2026-09-04: 人力分析在原通用序列结构内增加工资总成本、八个组成分项及已知部分，保留完整总成本键；响应、双期和导出统一裁剪四项敏感工资组成。分项独立零／未知判断，无新增路由或数据库迁移。
- 2026-09-04: 财务分析响应复用既有提示字段返回分类型全历史缺付款日期笔数及精确金额；存在未归期商务付款时完整合计保持未知并新增明确标注的已归期部分摘要。回款完整来源的真实空月为零，缺币种／日期／重复证据统一核验；无新路由、无数据库迁移，查询仍只读。
- 2026-09-03: 项目期间回款增加真实业务编号、银行回单号和独立凭证清单，新增按项目／期间／证据版本实时复核的只读预览入口；规范裁片优先且冲突不回退，原件严格同合同关联，路径及真实格式校验、禁止缓存和范围内元数据读取均保持金额与旧快照不变。
- 2026-09-03: 项目分析增加期间实际回款序列、整段摘要和明细来源字段；当前已确认未冲正事实按业务日直接归期，月季年可加，仅流量行与旧期末指标隔离，异常证据保持未知，原累计和月结快照不改，无新路由或数据库迁移。
- 2026-09-03: 当前月报页面移除旧“连续历史趋势与同期对比”折叠区及其全历史加载调用；旧趋势接口、类型和服务兼容保留，七模块不再依赖旧趋势可用年份，金额与权限不变。
- 2026-09-03: 同步季度独立 12 季窗口契约、一般账户权威总支出序列、项目期间明细字段及商务行政区／完整路径分组；补充四个可选快照维度、旧历史未知与不回填边界，无新增路由或数据库迁移。
- 2026-09-03: 五模块响应展示标题更名并同步双期导出名称，保留原模块键、查询参数、金额格式与权限，未部署生产。
- 2026-09-03: 余额模块响应标题统一为“账户余额资金台帐”并同步双期导出表名；接口字段及财务计算不变，未部署生产。
- 2026-09-02: 分析接口新增对比年份及顶层双期结果，同连接只读加载并分别保留筛选／未知状态；权限和内容版本覆盖双期。比较导出全量 29 表、单模块 5 表，无比较仍 15／3 表，未部署生产。
- 2026-09-02: 既有月报手工项目保存接口支持一般账户实际税费支付，服务端固定分类—账户—方向并校验正金额、当月真实日期、说明和凭证号；沿用权限、版本和月结锁，不新增接口或回填历史税款。
- 2026-09-02: 分析来源时间过滤非法值并按真实时刻比较，余额明细不再使用查询生成时钟；保持同一来源的查询／导出内容版本稳定，不修改原台账时间。
- 2026-09-02: 月报新增七模块分析与导出接口，使用同连接可重复读只读事务、精确角色复核、独立筛选和内容版本防错导出；新自动明细附带冻结分析维度并严格校验工资分项闭合，旧快照继续兼容。
- 2026-08-31: 月报扫描回单付款／收款户名改按 OCR（光学字符识别）坐标与同列账号绑定，修复工商银行双栏全文行序交错导致的户名对调、空收款户名误填；兴业文字层双名称同一行时按下一字段标签截断。文件摘要防重扩展为“报表月＋摘要＋识别版本”，识别规则升级可用同一原件生成新证据版本并保留旧版。
- 2026-08-31: 月报同号回单历史内容差异新增两阶段核实换版：首次上传仅创建一次性复核挑战并返回新旧字段清单，确认时要求同一管理员、同一会话重传完整原件批次并提交核实原因与逐笔差异摘要；文件、识别、月报版本或旧事实任一变化均拒绝，成功后旧证据永久保留并写独立纠错审计。
- 2026-08-28: 月度财务报表全部读取接口新增超级管理员，维护、银行上传、复核、月结和重开接口将超级管理员与普通管理员同权；使用精确角色中间件阻止董事长继承超级管理员的月报权限，相关裁片和历史回单预览同步开放超级管理员。

- 2026-08-28: 生产、开发容器及无环境变量回退的 OCR（光学字符识别）默认模型统一为 `v6_medium（第六版中型模型）`；接口路径、请求响应结构和业务安全门禁不变，第四版继续保留为显式回滚模型。

- 2026-08-28: 月度财务报表下载主表的福利金区移除补充收入展示列，结算区改为需求表规定的两行布局；实际金额及历史补充收入仍由公式和追溯明细计算。
- 2026-08-28: 月度财务报表响应和下载新增福利账户一饮用水、办公、电费三项金额；旧 407 通用费用读取时兼容并入办公，下载主表恢复需求表福利金分栏。

- 2026-08-27: 辅助材料整包删除能力在历史导入档案上与新建档案保持一致；仅管理员组可按当前版本删除，事务内写入 `auxiliary_package_deleted`（辅助档案已删除）审计后级联清理档案与文件索引，提交后再校验受控路径并清理物理文件。
- 2026-08-25: 月报基本账户工资回单按唯一员工姓名和完整账号逐笔接入薪资明细与实发工资付款凭证；不以系统实发工资差异阻断，金额差异仍由工资页面提示。已有凭证按回单号、员工、姓名账号和金额严格替换，原工资月份选错不阻断，同员工不同回单号可追加；上传与自动同步支持幂等补挂。
- 2026-08-25: 月报银行上传允许一般账户中凭证字段完整但备注关键词不足的回单继续执行合同严格核对；主营或资产唯一命中后只挂载并替换展示凭证，银行金额不覆盖系统业务统计，旧合同回单和匹配快照继续保留。
- 2026-08-25: 月报银行交易响应拆分员工匹配与报销关联并返回有效关联报销明细；裁片预览保留管理员／总经理读取能力，同时允许报销申请人读取本人有效关联裁片，越权与路径逃逸返回 `403`。
- 2026-08-25: 月报银行上传允许合同未关联交易单笔待核对，不再返回上传级合同匹配失败；安全利息和手续费继续计入，其余业务保留旧来源，文件部分状态继续阻止月结。
- 2026-08-25: 月报银行上传取消浏览器摘要硬门禁；客户端摘要缺失或失败时继续请求，由服务端执行摘要与版本防重，网络结果不确定状态仍支持刷新和人工安全重试。
- 2026-08-25: 完成月报银行回单终审收口：补齐请求前摘要保护与安全停止、直接月份选择和复核排除导航锁；中文大写金额兼容亿万、补零角分及无元小额写法，最终全量回归通过。
- 2026-08-24: 月报接口增至十个，新增待复核交易排除；合同候选加共享行锁，历史预览绑定文件版本，提交不确定时保留原件。工资及三类报销强制唯一员工匹配，金额、利息和页数门禁收紧。
- 2026-08-24: 月报查询实时叠加银行待复核状态，合同凭证冲正后主页面与月结接口同步禁用；中文银行文件名统一安全解码，自动数据查询改为事务内串行、事务外并行。
- 2026-08-24: 月报银行文件摘要改为同报表月内联合唯一，混月原件可按月份分别重传确认；不完整识别在替换旧活动文件前失败，主营收入和资产支出增加已确认合同严格匹配门禁，部分识别及待复核状态继续阻止月结。
- 2026-08-24: 月度财务报表增加银行回单查询、上传和单笔预览三个接口；支持三账户完整账号识别、文件及电子回单号防重、交易月份确认、同月同账户完整文件版本替换、严格业务证据链接和上传后原子刷新月报。
...
既有合同识别上传入口支持资产二级分类 `notary_fee`（公证费），仅允许独立主合同及 PDF（便携式文档格式）。拒绝补充协议、解除协议和上级合同关联；不得由客户端提交核心识别字段。识别任务成功时记录直接生效，前端跳转详情；普通提交审批接口拒绝公证费通知单草稿，避免利用普通人工确认流程替代专用校验。
...
- 2026-09-09: 董事长五类报销免审批、福利2分类与审批查询接口、双福利分类安全删除及付款角色边界正式上线。
- 2026-08-26: 合同列表接口新增关联补充协议数、终止／解除协议数和关联协议总数；资产履约新增统一 `costSettledAmount（支出合同履约结算额）`，保证金、电费等合同外费用不再混入完成率；所有合同分类在存在文件自身编号时均优先返回该编号，并继续保留系统内部编号。
- 2026-08-26: 历史财务凭证详情区分 `historical_confirmed_import（历史确认导入）` 与 PP-OCR（飞桨文字识别）自动结果；付款角色迁为科技最终对外付款时，月度银行分类及预览链接原子换绑。
...
- 2026-04-02: 财务区安全与一致性修复 - PUT /api/reimbursement/:id 补齐 is_deduction 字段写入；/:id/deduction-invoices POST 接口增加 filePath session 归属校验；核减发票删除前强制 validateFilePath；付款回单 OCR cache 绑定 targetType/targetId/verifierId/fileHash，提交时严格匹配并一次性消费；审批通过/驳回改为事务写入；商务报销 client/service_target 更新时同步双写；当时大额报销后端阈值统一为 >=1000（<1000 拒绝），该历史门槛已于2026-09-22取消
...
- 2026-08-31: 人力成本工资查询与修改接口新增 `withheld_actual_amount`（代扣实际发生额）和 `net_salary_actual_amount`（工资实际发生额）；空覆盖值默认返回对应账面值，成本合计按两项实际发生额汇总
...
- 2026-08-25: 报销普通发票与核减发票上传的本地识别支持读取多页 PDF（便携式文档格式），最终金额取完整发票中的“价税合计（小写）”；合同财务识别继续使用默认首页面范围，不改变其多页复核门禁。
...
- 2026-08-20: 主营项目新增在线三联单生成接口；工程项目名称和本次实际付款由申请人填写，合同金额与以前累计由服务端读取，累计金额按分计算并冻结快照，输出第一联、第二联、第三联三页 A4 PDF。历史 Excel 三联单检查、预览和打印接口继续兼容。
...
申请人角色允许 user（普通员工）、admin（管理员）、super_admin（超级管理员）和 chairman（董事长）。创建接口从登录会话确定申请人，编辑、材料维护、生成三联单、提交、撤回和删除仍核验申请人本人；scope=mine（本人范围）始终按当前账号过滤。审批接口仍只允许总经理。合同台账为上述四种角色返回批量开票资格，管理员的“全部”行政区合同按原读取权限参与资格校验。

开票申请与付款申请完全分离。请求中的 `amount`（本次开票金额）表示客户拟支付、我公司据此申请开票的金额，不得写入公司对外付款事实。

- `GET /api/invoice-applications/eligibility?contractId=...`：返回主合同摘要、当前有效合同金额、已开票金额、审批／盖章／待开票占用金额、剩余可申请额度和同一甲方最近一份非草稿、非驳回申请的有效开票信息。正常上传的主营／非主营主合同必须完成盖章核验归档；批量导入且带有 `historical_contract_imported`（历史合同已导入）审计的合同，在存在当前盖章合同文件且状态为生效中、执行中或已完成时按已归档兼容。资产合同、子协议、草稿、审批中、待盖章、撤销、驳回、终止和无剩余额度均拒绝。
- `GET /api/invoice-applications`：申请人使用 `scope=mine` 并以 `view=current|history` 区分当前与历史；总经理使用 `scope=manager_pending|manager_processed` 区分“待我审批／审批记录”；管理员使用 `scope=admin_pending|admin_processed` 区分“待我处理／处理记录”，旧 `scope=admin` 继续兼容待我处理。管理员待办是全体管理员共享的 `pending_seal`（待盖章）与 `pending_invoice`（待开票或待财务登记）队列；处理记录按 `delivered_by`（盖章交付人）或 `issued_by`（开具登记人）任一等于当前账号过滤，保留尚处于待开票／待财务登记及已经完成的记录，并按本人最近实际处理时间倒序。两阶段可由不同管理员处理，同一申请只返回一行；已处理但仍有共享后续任务时允许同时出现在两个范围。总经理和管理员范围均支持 `keyword`（关键词）在服务端分页前匹配合同名称、编号、甲方和申请人。`GET /api/invoice-applications/:id` 同步限制管理员只能查看共享待办、本人申请或本人实际处理记录。
...
- `POST /api/invoice-applications/:id/triplicate`：仅合同甲方全称为“国网北京市电力公司”的主营项目申请人在线生成固定三联单。页面允许填写工程项目名称和本次实际付款金额；服务端锁定合同链，读取当前有效合同金额，以“已开票金额＋尚未被正式发票覆盖的有效申请余额”作为之前累计，按分计算“之前累计＋本次＝本次累计”，校验本次金额不超过剩余可申请额度且累计不超过合同金额，并生成第一联、第二联、第三联三页 A4 PDF（便携式文档格式）。回款只作为财务佐证，不与正式发票重复相加。
- 2026-08-28: 开票资格与在线三联单统一累计口径：之前累计取已开票金额加未覆盖申请余额；本次金额由申请人按本次实际业务填写，系统只提供剩余额度并执行超额校验，不再把剩余额度误当成本次金额。
- `POST /api/invoice-applications/triplicate-inspection`：历史兼容接口。符合开票申请人角色的员工或管理员仍可对旧固定三联单 Excel（电子表格）执行只读结构检查；新申请页面不再要求上传该文件。
- `POST /api/invoice-applications/:id/materials` 和 `DELETE /api/invoice-applications/:id/materials/:materialId`：仅草稿所有人可增删材料。只有甲方全称为“国网北京市电力公司”的主营合同使用固定三联单，新申请在线生成三页 PDF（便携式文档），历史 XLS（旧版电子表格）或 XLSX（电子表格）三联单继续要求精确包含“其他费用”工作表并按分核对金额。其他主营和非主营材料可为 Excel（电子表格）、PDF、JPG（联合图像专家组格式）或 PNG（便携式网络图形），单份申请累计最多 20 份；也可按材料模式明确选择无需材料。文件进入扩展名、真实文件头、容器可解析性、大小、工作表数量和主动内容安全门禁；固定三联单金额来自文件结构，不调用 OCR（光学字符识别）。
- `DELETE /api/invoice-applications/:id?expectedVersion=...`：仅符合申请角色的申请人本人可删除状态仍为草稿且版本一致的开票申请。服务端在根合同和申请行锁内再次校验，不允许删除已提交、驳回或已产生正式发票分配的记录；事务删除材料、生成文件、草稿审计和申请记录，提交成功后再清理对应物理文件。
- `GET /api/invoice-applications/:id/materials/:materialId/preview`：受权预览。系统在线生成的固定三联单直接内联返回三页 PDF；历史 Excel 三联单仍只返回“其他费用”派生预览，绝不返回其他工作表。
- `GET /api/invoice-applications/:id/materials/:materialId/download`：下载完整原件，仅用于受权业务处理和审计；下载不得改写原文件。
- `GET /api/invoice-applications/:id/materials/:materialId/print`：仅对需要盖章的材料和管理员任务开放。系统生成的固定三联单以内联三页 PDF 返回，管理员可直接使用浏览器打印；历史 Excel 三联单继续导出只显示“其他费用”工作表的 PDF，转换不可用时安全回退为仅含该表的 XLSX（电子表格）。其他主营与非主营需盖章材料按原格式下载打印。
...
- `POST /api/invoice-applications/:id/deliver`：管理员登记材料已盖章并交付申请人，必须提交本次申请对应的 `sealedTriplicateFileId`。该文件须由当前管理员上传至同一合同、需要三联单时类型为“三联单”，其他甲方材料类型为“其他附件”，真实格式为 PDF（便携式文档格式）且仍为有效附件；图片或其他格式不能推进交付。校验通过后状态进入待开具发票。
...
- 2026-08-26: 合同台账汇总数量、根合同置顶排序和单行提醒标记统一使用未来1个月窗口；已完成且租赁结束日早于当天的历史合同从提醒判断中排除，生效中或执行中但已逾期未处理的合同仍继续提醒。
- 2026-08-26: `GET /api/contracts`（获取合同台账）不再因历史导入审计标记清空 `ownerName`（责任人姓名）；接口统一按合同 `created_by`（责任人账号）关联活动用户，本批历史合同返回吴静雯。
- 2026-08-26: `GET /api/contracts`（获取合同台账）默认按主合同 `contract_date`（合同日期）倒序分页，日期为空的合同置后；租赁到期提醒对财务角色仍具有最高置顶优先级，审批待办继续按提交时间正序处理。
- 2026-08-26: `GET /api/contracts`（获取合同台账）的 `counterparty`（对方单位）筛选改为按整个合同族匹配甲方、乙方、合同名称和项目名称；历史导入合同抬头包含具体供电公司而双方结构字段只记录上级主体时，也能正确检索。
- 2026-08-26: `GET /api/contracts`（获取合同台账）的 `name`（台账主名称）对全部合同分类统一只读取 `project_name`（合同内项目名称）；字段缺失时返回“项目名称待识别”或“—”，禁止使用合同标题、文件夹、文件名、租期及人为期次代替。
- 2026-08-25: 审批中心整包银行回单自动匹配与手工认领统一复用单张回单原子事务；接口在服务端复核唯一真实交易日期、整数分金额、收款人完整账号、付款批次快照和重复凭证，申请人统一按批次确认收款。
- 2026-08-25: 月报接口补齐一般、商务账户利息／跨行手续费逐笔自动明细并保留物理账户、回单号与预览；基本账户利息及手续费继续忽略，不进入字段、手工分类或公式。
- 2026-08-25: 月报银行上传与自动同步新增事务内报销补挂；响应区分员工匹配和具体报销关联，支持同日多回单合计及同类型组合，重复原件可直接重匹配，申请人可在有效本人链接下预览月底裁片。
- 2026-08-25: 付款回单相关接口统一规范回单号、事务咨询锁、行锁和条件更新；跨月换版、多批次身份冲突及并发双批次失败关闭，确定失败清理新文件，提交结果不确定时保留证据。
- 2026-08-25: 新增月报报销原回单专用预览接口；只从当前有效替换链接读取历史路径，支持发票目录、审批回单目录和月报银行目录，并限制为管理员、商务总经理或该报销申请人访问。
- 2026-08-25: 月报统计改为系统业务记录权威口径；银行回单只挂载主营、资产、报销和逐人工资凭证，利息／手续费例外按银行金额生成只读手工项目行并提供在线预览；公式版本升级为 `monthly-finance-v3-system-business-bank-evidence`。
- 2026-08-25: 银行上传和自动同步增加主营／资产整月重匹配；系统合同财务记录晚于银行文件补录时，可直接补挂并恢复文件状态，无需重新上传银行原件。
- 2026-08-25: 银行上传账户判定支持内部划转双账号方向消歧；严格付款回单选择付款账号、收款回单选择收款账号，方向标题缺失或冲突时仍返回账户无法唯一确定，不写入月报。
- 2026-08-25: 合同项目名称跨行规则补齐“强项目主体＋下一行前期手续技术咨询服务”结构；返回字段、候选证据及合同主表统一采用完整名称。第十八版只自动刷新逐行证据能证明存在遗漏续行的旧成功草稿，不扩大普通正文拼接范围。
- 2026-08-26: 合同财务登记创建与补充接口支持主营收入先保存实际回款、后补发票；无发票或回款暂时超过发票时仍确认银行事实并保持登记开放，后续发票和跨月回款继续写入原登记。每次补充都会重建全部发票与多笔回款的金额对应关系，只有两侧按分完全覆盖后才自动关闭整组登记。
- 2026-08-26: 付款回单整图遗漏主体时新增同模型受限增强复扫；双列坐标和付款／收款角色账号共同约束主体归属，唯一付款账号不得复制为收款账号，冲突、缺失或低置信继续失败关闭。合同银行回单解析版本升级为 `contract-bank-receipt-parser-v11`（合同银行回单解析第十一版），旧未消费任务自动升级重跑，接口字段结构不变。
- 2026-08-26: 科技对外付款登记修正按钮与接口分流：无开放登记的仅外付使用创建接口，已有登记追加外付使用登记编号外付接口，已有外付后仅补发票使用 `settlements`（登记补充接口）并提交空银行回单数组；旧任务刷新后按最终付款角色重新选择卡片和路由。
- 2026-08-27: 资产合同付款新增押金条闭环接口：`POST /api/contracts/:id/financial-records/:recordId/deposit-receipts` 上传 PDF（便携式文档格式）、JPEG（联合图像专家组格式）或 PNG（便携式网络图形）押金条并自动识别金额；`POST .../:depositReceiptId/verify` 由管理员确认识别金额或提交手工金额；`GET .../:depositReceiptId/file` 在线查看原件。详情接口新增 `depositReceipts`（押金条），付款记录返回 `confirmedDepositAmount`（已确认押金）和 `invoiceRequiredAmount`（应开票金额）；银行原始付款金额始终不改写。
- 2026-08-27: 押金条新增误上传处理接口：`DELETE /api/contracts/:id/financial-records/:recordId/deposit-receipts/:depositReceiptId` 只允许硬删除 `pending`（待确认）记录并同步清理物理文件、写入主合同审计；`POST .../:depositReceiptId/void` 只允许将 `confirmed`（已确认）记录撤销为 `voided`（已撤销），请求必须且只能提交 `{ reason }`。撤销响应及合同详情返回 `voidedBy`（撤销人）、`voidedByName`（撤销人姓名）、`voidedAt`（撤销时间）和 `voidReason`（撤销原因）；原件、识别值和确认值继续保留。
- 2026-08-27: 新增租赁押金台账及付款用途 API（应用程序接口）：`GET/PUT /api/contracts/:id/deposit` 管理押金条款、金额和资金来源，`POST /api/contracts/:id/deposit/settlements` 登记退款／扣款／抵租，`POST .../engineering-return` 登记实际退回工程资金，付款与科技对外付款的 `/purpose-details` 将银行原始金额按合同款、租赁押金精确拆分。支付日期只能取关联银行回单；撤销已确认押金条会同步撤回付款用途和押金支付状态，已发生结算时禁止撤销。
- 2026-08-27: 修复押金条确认时清理旧付款用途误带查询别名导致服务器错误的问题；确认事务现在使用受控列名删除旧拆分，原件、金额和待验证记录在失败时保持不变，可直接重试而无需重新上传。
- 2026-08-27: 租赁押金结算改为“回单识别＋识别任务确认”两阶段：原路退款与每次实际退工程均先调用专用识别接口上传文件，确认接口只消费通过安全门禁的任务；金额、交易日期和电子回单号只取服务端识别快照，退款、退工程累计值、回单元数据和主合同审计在确认事务提交。押金快照按结算返回单个 `refundReceipt`（押金退回回单）与多个 `engineeringReturnReceipts`（退工程回单），原件只允许财务角色通过受控地址在线预览。
- 2026-08-27: 历史退款结算回单补传接口 `POST /api/contracts/:id/deposit/settlements/:settlementId/refund-receipt` 改为只接收 `ocrJobId`（识别任务编号）；识别阶段绑定原退款记录，金额和日期必须与原记录完全一致，确认只补回单与审计，不更新结算金额或押金状态。
- 2026-08-27: 押金结算日期与退工程日期改为校验真实公历日期；押金及付款用途读取复用合同区域权限校验。抵租金结算会在同一事务把对应金额由租赁押金用途转为合同款、重新建立发票与付款对应关系；关联押金尚未解绑时，财务登记冲正被明确阻断。
- 2026-08-27: `PUT /api/contracts/:id/deposit` 支持操作顺序自动协调：若管理员先在付款行验证押金条、后建立押金台账，服务端按合同、确认金额和有效付款查找唯一已验证押金条并自动关联；唯一命中时同步写入支付日期、付款用途和保管中状态，多条同金额候选则停止并提示人工区分。
- 2026-08-27: 新增押金结算回单删除接口 `DELETE /api/contracts/:id/deposit/settlements/:settlementId/receipts/:receiptId`。接口按合同、结算、回单顺序加锁；押金退回回单仍有退工程回单时拒绝删除，退工程回单删除后按剩余证据重算累计金额、最后日期和操作人。删除审计与数据库变更同事务提交，明确提交成功后才清理物理原件，提交结果不确定时保留文件并要求刷新。
- 2026-08-27: 新增 `POST /api/contracts/:id/deposit/return-receipts/recognize` 与专用未消费任务删除接口。识别固定走 `PP-OCRv6_medium`（第六版中型模型）银行回单安全门禁，不调用 Tesseract（开源文字识别）；退款核对原付款反向主体和账号，退工程核对科技至工程方向，识别阶段及确认事务均复核金额上限、日期和电子回单号防重。专用任务以业务用途和目标编号隔离，不进入普通财务恢复、重试和登记消费。
- 2026-08-27: 押金结算回单识别补齐数据库提交结果不确定保护：任务登记或识别结果提交无法确认时保留物理原件，返回专用刷新提示且禁止重复上传；只有明确回滚且任务未保存时才清理本次上传文件，避免已提交任务指向缺失原件。
- 2026-08-27: 押金退款或退工程识别上传若命中同合同已经确认入账的发票、回款、付款或对外付款原件，接口保留 `409` 冲突且不创建第二个识别任务，并按既有记录类型提示“已入账”；响应同时使用当前押金上下文返回应上传的真实 `付款方→收款方` 方向。普通合同财务上传继续使用原有通用重复文件提示。
- 2026-08-27: 修复工程咨询内部划拨补入已有资产登记时错误叠加签约公司外付发票匹配金额的问题。内部划拨改为独立累计并允许先行入账，不受发票／外付金额硬门禁且不生成发票匹配；同批补票只重建签约公司外付匹配。服务端按资产资金模式、工程付款主体和动态签约公司收款主体限制未分配入账范围。
- 2026-08-27: 已完成资产合同新增内部划拨缺口专用接口：汇总、回单识别、未消费任务删除和批量确认四条路由仅允许管理员处理工程划拨科技模式的主合同；确认后只新增内部划拨付款，不改变合同完成状态、原财务登记或发票匹配关系。
- 2026-08-27: 报销普通发票与核减发票上传、创建、编辑、恢复及历史核减保存接口统一执行规范化发票号码和文件摘要双重查重；跨普通／核减表及并发请求均使用事务咨询锁，核减写入字段以服务端识别结果为准，旧独立核减增删仅允许草稿或已驳回单据。
- 2026-08-27: 报销发票与数据库既有记录重复时，响应统一携带原上传人及实际上传区域；普通发票固定提示“XX 已在发票上传中上传此发票，请勿重复上传”，主表核减发票和历史核减发票固定提示“XX 已在核减上传中上传此发票，请勿重复上传”。既有记录的号码与文件摘要命中在上传、创建、编辑、恢复及旧保存接口使用相同文案。
- 2026-08-27: 当前页面上传队列、并发识别队列或同一次创建／编辑请求内部出现重复发票号码时，区别于数据库既有记录，统一提示“发票号码 XX已在报销模块使用，请勿重复上传”；数据库既有记录仍显示原上传人及普通／核减上传区域。
- 2026-08-27: 普通发票编辑页上传接口接收当前报销编号并校验本人草稿／已驳回归属；若重复文件或号码属于当前正在编辑的报销单，按当前页面重复返回发票号码提示，属于其他有效报销单时仍返回原上传人提示。前端本地号码比较同步执行 NFKC（兼容性规范化）、去分隔符和大小写统一。
- 2026-08-27: 当前页面普通发票区与核减发票区交叉重复时进一步标明原所在区域：核减区重复普通发票提示“发票号码 XX已在发票上传报销模块使用，请勿重复上传”，普通区重复核减发票提示“发票号码 XX已在核减发票上传报销模块使用，请勿重复上传”；同一区域内部重复仍使用不带区域的报销模块提示。
- 2026-08-28: 月度财务报表新增只读趋势接口 `GET /api/monthly-financial-reports/trend?from=YYYY-MM&to=YYYY-MM`；超级管理员、普通管理员和总经理可查询最多 240 个月的已持久化月报趋势、缺月断点、可用年度和聚合提示。已月结月份沿用当前版本不可变快照，开放月份返回未月结的当前精确工作金额；主营实际到账与四账户结算流入、流出、净变化和期末资金分别保持原始十进制字符串。
- 2026-09-01: 月度财务趋势接口在没有月报快照的已发生月份，额外按合同台账的已确认、未冲正人民币主营回款生成“主营实际到账”来源点，不补建历史月报；已有月报值继续优先，其他资金指标保持断点。接口同时校验真实回款日期、当前月份边界和币种，外币缺少冻结汇率时不生成精确点并返回提示，可用年度合并月报年份与有效主营回款年份。
- 2026-09-01: 财务趋势前端新增“查看年度”和“完整历史”双模式；完整历史从最早可用主营回款年度连续查询至最新年度，并按趋势接口单次最多240个月的限制自动分段、每批最多并发3段后按月份去重合并，不调整接口权限、金额口径或服务端范围门禁。
- 2026-09-01: 财务趋势响应新增主营业务行政区列表及逐月分区数据，包含已确认实际回款、按主合同签订月归属的当前有效合同额和主合同链数量。无记录月份返回明确零值并参与折线连接；非人民币回款、上传日期代替合同日期等不确定来源返回空值及提示，不伪造零值。合同数量只计主合同根节点，补充和终止协议不重复计数。
- 2026-09-02: 合同财务登记补充接口仅收到新发票时，复用扣除已登记押金后的对外付款匹配逻辑；无新增银行回单时跳过增量分配，返回新票匹配并更新累计分配审计，避免金额本来一致却出现服务器错误。请求结构、权限及原有事务回滚规则保持不变。
- 2026-09-07: 住房全成本开发版增加人力公司分摊前构成字段及独立住房构成导出，固定租金物业按租期、变量按原票收费月并核对票号和整票价税合计；未知不补零、押金和其他资产不计费用，无业务写入接口，尚未部署，待最终验收。
- 2026-09-07: 住房全成本正式副本月／季／年、单人单月、工资权限及33表导出验收通过；14张原票收费月核验通过，其他六模块与最新生产基线完全一致。55张3,449行业务表及生产镜像未变，尚未上线。
- 2026-09-07: 住房变量费用的人员分摊来源说明增加所属租赁合同，导出补充跨合同计提规则；复核旧合同到期／新合同接续、签订日与起租日不同、独立地址真实重叠、物业零与空值的区分，不变更接口结构或原始业务数据。
- 2026-09-07: 正式来源启用发票全租期口径，新增housingCostBasis（住房口径标识）并在本期／同比合并中保留；读取全部有效发票而非仅变量票。21张发票全租期461,129.58元精确闭合，本期住房157,921.09元，隔离接口及33表导出通过，未改业务数据或上线。
- 2026-09-07: 发票全租期口径正式发布；最新生产副本的启动、月季年、单人、33表导出与其他六模块严格一致性通过，生产切换前后及页面验收后55张原始业务表不变。
- 2026-09-08: 出账新增chartMetricKeys（可选指标白名单），行政合并基础／大额、手续费仅一般账户有效跨行费用；完整性按来源独立判断。8月9.00元手续费、10,331.05元已归期行政与导出闭合，30笔未归期报销不猜日期，其他六模块不变，尚未上线。
- 2026-09-08: 出账分类与白名单已发布，最新正式副本启动／月季年／明细／导出及其他六模块一致性通过；切换前后及生产页面验收后55表不变，未归期付款保持待核对。
- 2026-09-08: 开发统一确认报销来源供行政、商务与人力分析，新增两类模块口径标识；正式68笔无缺月、无重复，月季年逐期公式、范围构成及33表导出通过。生产未切换。
- 2026-09-08: 统一确认报销来源正式发布；最终镜像两服务最小替换，最新正式副本及生产页面逐期闭合，切换前后与页面验收后55张业务表不变。
- 2026-09-09: 开发`categoryTotal`（五类统计总金额）及分类`knownValues`（已知金额数组）／`partial`（完整性标记）字段；正式月度906,335.52元、季度及年度精确闭合，原现金序列及其他六模块一致，尚未发布。
- 2026-09-09: 五类统计总金额接口已发布生产；最终镜像只替换分析服务与活动财务页面脚本，正式副本、导出和生产资源校验通过，原银行序列、其他六模块及55张相关业务表保持不变。
- 2026-09-08: 人力成本回单服务仅为个税分类启用工行国库遮挡账号校验；个税识别版本升至 4，复用月份查询的历史回单升级队列，无新增接口或返回字段。
- 2026-09-08: 个税遮挡账号识别修复已发布生产；244 套、3,378 项完整测试通过，生产原图完整识别 302.41 元，服务健康及切换前后业务数据摘要核验通过。发布与回滚资料见 `backup/tax-receipt-release-20260908/`。
...
- 2026-09-08: 社保入口新增工行国库缴款遮挡账号兼容，复用大小写金额、收款单位、账号栏归属、用途和交易凭证校验；社保版本升至 5 并重识别旧版本，保留电子回单号事务查重。费用归类沿用用户选择，不根据通用国库用途推断费种。
...
- 2026-09-08: 修复国库回单文字错行导致误拒绝：保留账号数字换行边界，在金额栏内独立提取唯一小写金额并拼接被标签或小写金额打断的大写金额，仍核对到分且禁止从摘要备注借值；社保版本升至 6。
- 2026-09-08: 社保真实附件错行修复已上线；一至七月七条历史社保回单按已核验原件及电子回单号事务恢复为已识别、版本6，其他已核对业务数据保持不变；保留生产备份及逐条审计。
- 2026-09-09: 新增福利1分类、董事长四类报销免审批、本人收款资料及福利1查询字段接口；付款和银行回单写操作收紧为管理员与超级管理员精确角色。
- 2026-09-09: 新增福利2报销及独立分类接口，董事长五类报销统一免审批；福利1、福利2分类删除统一检查报销与月报手工引用，未引用才物理删除并保存删除标记，已引用返回409并可改为停用。
...
- 2026-09-14: 合同下载待办允许超级管理员受控代执行唯一管理员名下尚未认领的任务；认领后禁止抢占，详情、申请链、处理历史和导出均按本人实际执行账号隔离；同日完成生产上线验收。
- 2026-09-10: 辅助材料归档接口新增其他材料、逐文件删除和八类安全文件格式；首次归档不再强制要求辅助合同，辅助发票和回单继续固定不进入财务核算。
- 2026-09-10: 合同上传元数据新增非主营支出二级分类；新建草稿按锁定分类写入支出方向，财务识别、登记、台账汇总和经营看板统一按实际收入／支出方向校验与统计，非主营收入和其他服务原行为不变。
- 2026-09-10: 合同识别、详情、台账、补充／解除协议上下文及用印申请接口新增可选 `partyC`（丙方单位）；对方单位和关键词筛选同步覆盖丙方。财务主体在甲乙丙三方中仍必须唯一命中一家公司，双方合同响应和校验规则保持不变。
- 2026-09-10: 新增 `PATCH /api/contracts/:id/target-amount`（合同目标金额设置接口）；管理员可首次设置目标金额、目标数量、单价和当前确认值，后续变更必须提交原因并校验合同版本，详情接口同步返回不可变的目标金额变更历史。

## 公证费通知单接口兼容

合同元数据接口的资产类二级分类包含 `notary_fee`（公证费）。既有合同识别上传接口仅在“资产类＋公证费＋独立主合同＋PDF（便携式文档格式）”同时满足时进入专用识别；补充协议、解除协议、上级合同关联和非 PDF 文件均在服务端拒绝。识别字段由服务端生成，客户端不能提交核心字段绕过校验。普通合同提交审批接口也会拒绝公证费草稿，防止跳过专用识别门禁。

## 更新日志（续）

- 2026-09-10: 保留公证费付款通知的元数据、上传限制、专用识别和禁止普通审批规则；补齐无数据库回归测试。
- 2026-09-11: 合同元数据的新建非主营选项仅返回收入、支出两项；台账与经营看板接口新增受控 `declaredSubtype`（声明二级分类）筛选，收入筛选兼容历史其他服务记录，支出筛选只匹配非主营支出。经营看板响应分别返回非主营收入、支出的合同数、合同额及回款／付款指标。
- 2026-09-11: 合同银行回单解析升级至第十二版；批量新建、补充及科技对外付款均只读取已验证快照中的人民币币种和受控正文证据，取消无条件写入人民币。旧版三张本月回单经原件目视核对后仅按不可变文件摘要兼容，其他空币种记录仍阻断。
- 2026-09-14: 合同下载待办向超级管理员开放受控代执行；目标仍为唯一活动管理员，超级管理员只可认领尚未认领任务，认领后按真实执行账号互斥，处理历史与 Excel（电子表格）导出仅返回本人实际执行记录；同日完成生产上线验收。
- 2026-09-14: 开发与生产接口源码完成三方归并；非主营支出、三方合同、目标金额、历史合同、社保版本 6、福利双分类和月度财务接口同时保留。266 套、3,640 项完整测试及生产数据库副本启动验证通过。
- 2026-09-14: 修复并发布非主营支出创建及追加部分付款时误用资产资金方式门禁的问题；只有资产合同付款要求资金方式已确定，非主营支出继续严格核对发票购买方与回单付款人、发票销售方与回单收款人，主体不一致仍拒绝入账。267 套、3,644 项完整测试及生产数据不变验收通过。

- 2026-09-15: 开票资格合同摘要与申请详情新增 requiresTriplicate（是否需要三联单），仅主营且合同甲方全称去除首尾空白后等于“国网北京市电力公司”时为真；不包含分公司，不接收客户端覆盖。草稿材料方式、金额锁定、上传、生成、提交、预览和打印使用相同规则，既有申请读取甲方快照。其他主营项目可选择无材料、材料无需盖章或材料需盖章。交付接口保留 sealedTriplicateFileId（盖章文件编号）兼容字段，需要三联单时校验 triplicate（三联单）附件，否则校验 other（其他附件），均核验同合同、当前上传人及文件格式。

- 2026-09-15: 开票申请人权限扩展至管理员组，所有写接口保留本人、状态和版本门禁；合同台账批量资格同步向管理员返回，审批仍仅总经理。管理员待办响应 employeeActionPending（本人驳回待修改数）按当前申请人统计，adminPending（执行待办数）口径不变，total（总数）合并两者；未增加数据字段或迁移。
- 2026-09-15: 开票列表增加 `admin_pending`（管理员待处理范围）与 `admin_processed`（管理员已处理范围）；处理记录按盖章交付人或开具登记人隔离本人，返回两个阶段的真实处理人、时间和说明。管理员详情权限同步限制为共享待办、本人申请或本人处理记录，旧 `admin`（管理员范围）继续兼容待办查询；接口已发布生产并通过正式域名健康检查。
- 2026-09-15: 新增 `GET /api/invoice-applications/pending-receipts`（读取待上传回单）、`GET /api/invoice-applications/pending-receipts/:registrationId`（读取指定登记待回单进度）与 `GET /api/invoice-applications/:id/financial-progress`（读取开票财务进度）。前两者仅向管理员组返回当前未闭合收入登记的合同、申请、发票、有效匹配回款及剩余金额；财务进度只允许申请人本人，或已进入开具财务阶段的管理员读取，且只返回状态和金额，不扩大完整详情权限。待办数量响应新增 `pendingReceipt`（待上传回单数），`adminPending`（管理员全部待办数）和总数同步计入，开票与用印页签仍单独使用盖章及开票执行数量。分页查询先形成同条件总数再左联分页结果，请求页超出当前末页时仍返回真实总数和空列表，前端可据此回到有效页。
- 2026-09-15: 已完成收入合同的财务识别和登记增加服务端门禁：继续已有草稿登记时允许主营与非主营收入补齐回单；追加正式发票仍必须存在同合同已登记开具、仍为待财务登记的申请，已有待回单登记不能单独放开额外发票。解除协议提交审批及盖章归档新增未闭合财务登记检查，命中时返回 `CONTRACT_TERMINATION_OPEN_FINANCIAL_REGISTRATION`（解除合同存在未闭合财务登记）并保持合同及登记原状态。
- 2026-09-15: 合同台账响应中的待审批、待盖章数量改为忽略生命周期状态筛选并按主合同链去重，其他筛选继续生效；分别使用 `status=approving`（审批中）与 `status=pending_seal`（待盖章）下钻时，分页总数与所点击指标口径一致。
- 2026-09-15: 上线后边界加固：待回单越界页保留真实总数；已完成收入合同的开放登记只放行回单补齐，追加发票仍要求存在已登记开具且待财务登记的申请。纯回单与发票加回单的混合提交按实际新增凭证类型重新执行合同状态门禁。
- 2026-09-15: 合同开票财务进度、待上传回单及台账状态下钻接口已完成生产发布；正式未登录门禁、容器内只读服务、越界分页真实总数、生产构建文件和反向代理健康均通过验证，未执行正式业务写入。
- 2026-09-16: 合同详情接口新增管理员专属开票申请材料分组；`GET /api/contracts/:id/invoice-application-materials/:materialId` 以合同范围校验申请材料并执行摘要复核、在线预览和管理员下载。已关联盖章材料从普通 `files`（合同文件）和员工下载候选中排除，通用合同文件地址也禁止非管理员读取该类材料。
- 2026-09-16: 开票申请材料分组与受控预览接口已完成生产发布；申请关联上传仅接受真实 PDF（便携式文档格式），无申请编号的三联单由服务端拒绝，盖章文件仅在交付完成后进入归档，原始表格下载保持原文件。
- 2026-09-17: 开放月报读取实时合并福利1、福利2分类主数据；启停、名称和顺序无需同步自动数据即可生效，快照金额及已月结版本保持不变。本轮开发完成并通过专项回归，尚未上线。
- 2026-09-17: 开放月报福利分类实时联动已上线；最终镜像仅变更月报服务与路由，9月两项启用、8月月结快照、正式只读接口及126张业务表不变校验通过。
- 2026-09-18: 病假退出强制固定额度名单，余额检查支持自由开启或关闭；服务端尊重管理员配置，附件校验保持不变。
- 2026-09-18: 普通与组合请假提交接口新增独立返岗补假业务类型；仅允许补录今天或过去日期，父申请为空，审批、余额、附件、撤回、驳回和重提沿用现有接口与事务规则。
- 2026-09-18: 草稿重提接口新增已有附件删除编号；只允许删除当前草稿附件，并在重提事务成功后安全清理无引用物理文件，必传材料、历史版本和越权编号均受服务端保护。
- 2026-09-18: 请假上传中间件统一处理文件大小、数量和格式错误；单文件超过 5MB 返回 `413` 中文提示，前端选择阶段同步拦截，消除重复的通用服务器错误。
- 2026-09-18: 请假草稿再次送审改记 `submit`（提交申请），驳回后新版本继续记 `resubmit`（重新提交）；同一申请编号的附件只展示在最后一个提交节点。
- 2026-09-18: 启动迁移回填历史草稿日志：同一申请中紧邻撤回且时间唯一递增的旧 `resubmit` 改为 `submit` 并清理冗余备注；驳回后新版本、并列时间及异常序列不受影响。
- 2026-09-18: 请假详情接口继续返回完整日志；待审批时间线仅展示最后一次撤回后重新形成的有效审批周期，历史撤回周期不重复占用审批进度。
- 2026-09-18: 新增仅总经理可用的 `GET /api/leave/employee-statistics?year=YYYY`；当前总经理本人按 `user_id`（用户编号）进入目标年及此前四年的员工主集、总体对比、排名、假期类型分布和逐笔明细，本人请假仍由董事长审批，其他总经理明确排除；响应新增固定 12 项的 `monthlyComparison`（月度对比）、员工 `monthlyTrend`（月度趋势）及月度假期类型汇总，支持年度／月度点选下钻和筛选联动；每个员工返回含不限额度类型的 `balanceSummaries`（逐类型余额明细），排名响应不预先截断员工；五年申请与节假日回退分别采用一次区间查询，额度、趋势及下钻全程纯读取，并继续执行版本叶子、组合按年／按月去重和跨年年度分配口径；月度天数以有效年度分配快照为守恒目标，当前日历半天过多则截取、不足则按时间补足。
- 2026-09-18: 请假详情支持返回并展示 `historical_import`（历史导入）审批节点及其证明附件；页面操作人统一显示“系统导入”，不展示实际执行导入的管理员身份；此类线下已批准记录按无父申请的返岗补假读取，不伪造线上提交或审批动作。
- 2026-09-18: 总经理审批记录新增本人审批范围内的历史导入申请；按申请 `approver_id`（审批人编号）与历史导入日志双重限定，列表显示“历史导入”，详情及证明附件继续可查看，不扩大其他总经理的数据范围。
- 2026-09-18: 统计页面顶部改为紧凑年度标题与摘要，不再展示总经理姓名；统计接口的负责人字段与数据范围保持兼容。
- 2026-09-18: 请假审批记录新增可选关键词搜索，覆盖姓名、部门、事由、申请编号和假期类型；分页总数与列表共用筛选条件，保留本人审批及历史导入范围、最终版本过滤，并补齐同时间记录的稳定排序。
- 2026-09-18: 请假返岗补假、附件管理、统计与搜索优化已上线生产；追加3条历史导入后，正式审批记录4条（含原有1条）、年度与月度合计14天、姓名搜索及病假证明受控下载均通过实际路由只读验收，业务数据读取前后摘要一致。
- 2026-09-19: 人力资源审批工作台精确开放给总经理和超级管理员；超级管理员可用本人签名处理活动总经理名下的转正主管任务，并保留本人原人事部任务，不能代签董事长最终审批。待签计数和本月已审批按实际主管／人事部环节合并；新版签署提交携带预期环节和表单版本，超级管理员必填，事务锁内拒绝过期阶段或版本。
- 2026-09-19: 请假待办、审批、记录与员工统计精确扩展至超级管理员；代办仅覆盖活动总经理名下的非总经理申请并排除本人，原审批人不改写、日志记录实际超级管理员，原总经理可查看代审记录。待办计数与列表同口径，总经理请假仍仅由董事长处理；超级管理员统计合并全部活动总经理、普通员工和管理员范围，不新增数据库结构。
- 2026-09-19: 超级管理员人力资源接口已正式上线；正式只读验收确认超级管理员与总经理均返回 6 名员工、4 次已批准申请、14 天，普通管理员和董事长统计及普通管理员请假待审批列表继续返回 `403`，待办计数与列表一致，历史病假附件摘要一致。生产验收未执行审批写入，全部业务数据摘要保持不变。
- 2026-09-20: 开发新增请假逐人抄送通知、未读筛选及带通知版本的按条已读接口；统一待办增加精确管理员／超级管理员的 `leaveCcUnread`（抄送未读数量），草稿、旧申请版本及他人已读互不干扰，既有审批权限和流程不变；不回填历史提醒，尚未发布生产。
- 2026-09-20: 请假抄送接口进入授权发布验收，完整 291 套、3,927 项测试及类型检查通过；计划仅向当日目标申请的两名实际抄送人补发，正式验收只读检查未读入口，不替接收人清除提醒，上线结果待终验确认。
- 2026-09-20: 请假抄送接口已正式上线；当日目标申请仅新增两条通知，随后只读核验两名接收人的角标、未读列表和详情全部通过，两条提醒仍未读，未执行生产已读确认或修改审批状态。
- 2026-09-21: `GET /api/daily-logs/team` 同时合并正式归档和非空已保存日志，响应新增 `state`（归档状态）及独立 `weekDays`（完整周统计）；月份末日、跨月周、上海业务日期和在职人员口径统一修正。
- 2026-09-21: 团队日志接口修复已上线生产；正式返回 6 名应填人员、今日 3 名已填写，并按完整周返回 09-20 调休统计，周报接口同步读取补算后的 3 份周报。
- 2026-09-21: 历史周报零删除修复完成；团队周报接口继续按周起止精确查询，53 份正文与同人同周真实日报重新一致，3 份无本周来源记录未覆盖、未删除。前端只接受周范围与当前请求完全一致的最新响应。
- 2026-09-22: 团队日志图片展示正式复用 `GET /api/daily-logs/team/attachments/:attachmentId/preview`；缩略图和大图均按附件编号鉴权读取，生产真实图片返回 `image/png` 且与原文件摘要一致。
- 2026-09-22: 取消大额报销创建、更新和提交接口的1000元最低金额校验，并移除个人大额报销列表的1000元金额过滤；通用金额及发票校验保持不变。
- 2026-09-22: 大额报销无最低金额门槛接口已正式上线；生产服务端构建仅删除创建、列表和编辑重提三处门槛，健康检查、反向代理和报销相关数据不变核验通过。
