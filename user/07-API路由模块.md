...
| `GET` | `/api/monthly-financial-reports/:month/bank-receipts` | `admin`、`general_manager` | 查询基本、一般、商务三账户当前文件、分类汇总、异常和单笔回单 |
| `POST` | `/api/monthly-financial-reports/:month/bank-receipts` | `admin` | 以 `multipart/form-data（多段表单）` 上传一至三份原始银行 PDF（便携式文档），识别并原子刷新月报 |
| `POST` | `/api/monthly-financial-reports/:month/bank-transactions/:transactionId/review` | `admin` | 必填原因后复核排除待复核交易，保留原件与审计并重新计算月报 |
| `GET` | `/api/monthly-financial-reports/bank-transactions/:transactionId/preview` | 已登录用户；`admin`、`general_manager` 可读全部，其他申请人仅可读本人有效报销链接 | 内联预览整月银行原件中裁出的单笔回单 |
| `GET` | `/api/monthly-financial-reports/bank-transactions/:transactionId/reimbursements/:reimbursementId/previous-proof` | 已登录用户；`admin` 可读全部，`general_manager` 仅可读商务报销，其他申请人仅可读本人报销 | 通过当前有效替换链接内联预览被替换的原付款回单；`index` 指定多文件序号 |

月份必须为 `YYYY-MM`。所有写接口必须携带 `expectedVersion`（预期版本），使用月份级事务锁和可串行化事务；版本不一致、首次并发创建及序列化冲突统一返回 `409` 和 `MONTHLY_FINANCE_VERSION_CONFLICT`。未持久化月份必须先保存或同步，不能直接月结。总经理的任何写请求，以及超级管理员、董事长、BOSS（经营管理）和普通员工的十个接口请求，均在服务端权限中间件处拒绝。

银行上传按真实文件头再次校验 PDF（便携式文档），单份上限 50MB／120页，本批合计上限200页，一页最多上下两张，原文件不合并。完整账号只允许基本账户 `0200049609201258271`、一般账户 `0200303519000018418`、商务账户 `321240100100245908`；文件名只作为异常提示。文件摘要按“报表月＋摘要”联合查重；部分重复原件跳过，全部重复原件不新增版本但继续执行报销重匹配，只有并发唯一约束等无法安全确认的重复冲突才返回 `MONTHLY_BANK_FILE_ALREADY_UPLOADED`。同一混月原件可切换到其他月份重新上传并确认；单一银行交易月份与当前月不一致时返回 `MONTHLY_BANK_REPORT_MONTH_MISMATCH`；文件含多个交易月份且未确认时返回 `MONTHLY_BANK_MIXED_MONTH_CONFIRMATION_REQUIRED`。管理员确认后只生成当前报表月的交易事实，其他月份不跨月写入，页面提示切换到对应月份重新上传并确认。电子回单号已存在但完整账号、日期或金额不一致时返回 `MONTHLY_BANK_RECEIPT_CONTENT_CONFLICT`，整个事务回滚，不生成第二套数据。
...
- 2026-08-28: 月度财务报表新增只读趋势接口 `GET /api/monthly-financial-reports/trend?from=YYYY-MM&to=YYYY-MM`；管理员和总经理可查询最多 240 个月的已持久化月报趋势、缺月断点、可用年度和聚合提示。已月结月份沿用当前版本不可变快照，开放月份返回未月结的当前精确工作金额；主营实际到账与四账户结算流入、流出、净变化和期末资金分别保持原始十进制字符串。
