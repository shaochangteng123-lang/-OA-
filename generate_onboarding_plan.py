...
def create_onboarding_plan():
    ...
    # 基础报销
    add_paragraph(doc, '（1）基础报销（日常费用）', bold=True)
    add_paragraph(doc, '适用范围：日常费用报销，按现有基础报销规则办理')
    ...
    # 大额报销
    add_paragraph(doc, '（2）大额报销（不设金额限制）', bold=True)
    add_paragraph(doc, '适用范围：大额报销不设最低或最高金额门槛')
