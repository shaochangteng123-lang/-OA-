...
async function renderDocument() {
  ...
  try {
    ...
    if (!context) throw new Error("浏览器无法显示用印申请单");
    ...
    if (
      ...
      documentError.value =
        error instanceof Error ? error.message : "用印申请单加载失败";
    }
  } finally {
    if (sequence === renderSequence) {
      activeRenderTask = null;
...
.approver-signature-slot {
  ...
  gap: 5px;
  overflow: hidden;
  border: 2px solid rgb(28 141 126 / 70%);
  border-radius: 10px;
  background: rgb(239 250 247 / 86%);
  color: #116d63;
  cursor: pointer;
  font: inherit;
  text-align: center;
...
.approver-signature-slot:hover:not(:disabled),
.approver-signature-slot:focus-visible {
  border-color: #0f8275;
  background: rgb(232 248 244 / 94%);
  box-shadow: 0 0 0 4px rgb(15 130 117 / 15%);
  outline: none;
}
.approver-signature-slot:disabled {
  cursor: not-allowed;
}
.approver-signature-slot.is-signed {
  border-style: solid;
  background: rgb(232 248 244 / 92%);
  opacity: 1;
}
.approver-signature-slot strong {
  font-size: clamp(11px, 1.45vw, 16px);
}
.approver-signature-slot span {
  font-size: clamp(9px, 1.05vw, 12px);
}
.approver-signature-slot img {
  width: 52%;
  max-height: 48%;
  object-fit: contain;
}
