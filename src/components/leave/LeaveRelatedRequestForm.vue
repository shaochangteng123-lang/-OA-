<template>
  <div class="related-request-form">
    <el-descriptions :column="2" border size="small" class="parent-summary">
      <el-descriptions-item label="主申请">
        {{ parentRequest.request_no }}
      </el-descriptions-item>
      <el-descriptions-item label="原假期">
        {{ parentRequest.leave_type_name }}
      </el-descriptions-item>
      <el-descriptions-item label="原时间" :span="2">
        {{ parentRequest.start_date }}{{ halfLabel(parentRequest.start_half) }}
        至
        {{ parentRequest.end_date }}{{ halfLabel(parentRequest.end_half) }}
      </el-descriptions-item>
      <el-descriptions-item label="原天数">
        {{ parentRequest.total_days }} 个工作日
      </el-descriptions-item>
      <el-descriptions-item label="原事由">
        {{ parentRequest.reason || '未填写' }}
      </el-descriptions-item>
    </el-descriptions>
    ...
    <el-form
      ...
      :rules="rules"
      label-width="90px"
      class="related-form-fields"
    >
      <el-form-item label="申请方式">
        <el-radio-group v-model="requestMode" @change="handleModeChange">
          <el-radio-button value="single">单一{{ relationLabel }}</el-radio-button>
          <el-radio-button value="combined">组合{{ relationLabel }}</el-radio-button>
        </el-radio-group>
      </el-form-item>
      ...
      <el-form-item label="开始时间" prop="startDate">
        <div class="date-half-row">
          <LeaveDatePicker
            ...
            placeholder="选择开始日期"
            :disabled="relationType === 'extension'"
      ...
      <el-form-item label="结束时间" prop="endDate">
        <div class="date-half-row">
          ...
          />
          <el-radio-group v-model="form.endHalf" @change="onEndDateChange">
            <el-radio-button value="morning">上午</el-radio-button>
            <el-radio-button value="afternoon">下午</el-radio-button>
          </el-radio-group>
        </div>
      ...
      <el-form-item v-else label="假期分配">
        <div class="combination-editor">
          ...
          >
            <div class="segment-main-row">
              ...
              >
                ...
                >
                  <span>{{ type.name }}</span>
                  <span
                    v-if="type.requires_balance_check && getBalance(type.code)"
            ...
            <el-input
              v-if="reasonRequiredForType(segment.leaveTypeCode)"
      ...
      <el-form-item v-if="requiresAttachments" label="证明文件">
      ...
      <el-form-item>
        <el-button
          ...
          @click="handleSubmit"
        >
          {{ requestMode === 'combined' ? `提交组合${relationLabel}` : `提交${relationLabel}申请` }}
        </el-button>
...
import {
  ...
  getLeaveTypes,
  getMyBalances,
  getRelatedLeaveContext,
  submitRelatedLeaveRequest,
  type LeaveBalance,
  type LeaveRequest,
...
const props = defineProps<{
  sourceRequest: LeaveRequest
  relationType: 'extension' | 'supplement'
}>()
...
const parentRequest = ref<LeaveRequest>(props.sourceRequest)
...
const form = ref({
  leaveTypeCode: props.sourceRequest.leave_type_code,
  startDate: '',
...
const relationLabel = computed(() => props.relationType === 'extension' ? '续假' : '补假')
...
const selectedType = computed(() => getType(form.value.leaveTypeCode))
const reasonRequired = computed(() => reasonRequiredForType(form.value.leaveTypeCode))
...
const allocationStatusText = computed(() => {
  ...
  if (allocationDifference.value > 0) return `还需分配 ${allocationDifference.value} 天`
...
const rules = computed<FormRules>(() => ({
  ...
  reason: requestMode.value === 'single' && reasonRequired.value
    ? [{ required: true, message: '请填写请假事由', trigger: 'blur' }]
    : [],
}))

function halfLabel(half: 'morning' | 'afternoon'): string {
  return half === 'morning' ? '上午' : '下午'
}

function reasonRequiredForType(typeCode: string): boolean {
...
function initializeSupplementStart(request: LeaveRequest) {
  if (request.end_half === 'morning') {
    form.value.startDate = request.end_date
    form.value.startHalf = 'afternoon'
  } else {
    form.value.startDate = addCalendarDay(request.end_date)
    form.value.startHalf = 'morning'
  }
...
function disableStartDate(time: Date): boolean {
  if (props.relationType === 'extension') return isPastLeaveDateDisabled(time)
  const date = [
    time.getFullYear(),
    String(time.getMonth() + 1).padStart(2, '0'),
    String(time.getDate()).padStart(2, '0'),
  ].join('-')
  return parentRequest.value.end_half === 'afternoon'
    ? date <= parentRequest.value.end_date
    : date < parentRequest.value.end_date
}
...
function disableEndDate(time: Date): boolean {
  ...
  if (date < form.value.startDate) return true
  return props.relationType === 'extension' && isPastLeaveDateDisabled(time)
}
...
function handleModeChange() {
  ...
  if (requestMode.value === 'combined') {
    if (!segments.value.some(segment => segment.leaveTypeCode) && form.value.leaveTypeCode) {
      segments.value[0].leaveTypeCode = form.value.leaveTypeCode
      handleSegmentTypeChange(segments.value[0])
      return
    }
  ...
  calcTimer = setTimeout(async () => {
    try {
      const result = await calculateDays({
        ...
        allowPast: props.relationType === 'supplement',
      })
...
function calculateSelectedPeriod() {
  if (!form.value.startDate || !form.value.endDate) {
    calculatedDays.value = null
    return
  }
...
function validateCombination(): boolean {
  if (segments.value.length < 2) {
    ElMessage.warning(`组合${relationLabel.value}至少需要两种假期`)
    return false
  }
  if (segments.value.some(segment => !segment.leaveTypeCode)) {
  ...
  const missingReason = segments.value.find(
    segment => reasonRequiredForType(segment.leaveTypeCode) && !segment.reason.trim()
  )
  if (missingReason) {
...
async function handleSubmit() {
  ...
  try {
    ...
    formData.append('requestMode', requestMode.value)
    ...
    const result = await submitRelatedLeaveRequest(props.sourceRequest.id, formData)
    ElMessage.success(
      requestMode.value === 'combined'
        ? `组合${relationLabel.value}已拆分为 ${result.requests.length} 条申请`
        : `${relationLabel.value}申请已提交，等待审批`
    )
    emit('submitted')
  } catch (error: any) {
    ElMessage.error(error?.response?.data?.message || `${relationLabel.value}申请提交失败`)
  } finally {
...
onMounted(async () => {
  try {
    const [types, balanceList, context] = await Promise.all([
      getLeaveTypes(),
      getMyBalances(),
      getRelatedLeaveContext(props.sourceRequest.id),
    ])
    leaveTypes.value = types
    balances.value = balanceList
    parentRequest.value = context.parentRequest
    if (getType(form.value.leaveTypeCode)?.is_available === false) {
      form.value.leaveTypeCode = ''
    }

    if (props.relationType === 'extension' && context.suggestedExtensionStart) {
      form.value.startDate = context.suggestedExtensionStart.date
      form.value.startHalf = context.suggestedExtensionStart.half
      form.value.endDate = context.suggestedExtensionStart.date
      form.value.endHalf = 'afternoon'
    } else {
      initializeSupplementStart(context.parentRequest)
    }
    onDateChange()
  } catch (error: any) {
    ElMessage.error(error?.response?.data?.message || '加载关联申请信息失败')
  }
})
</script>
